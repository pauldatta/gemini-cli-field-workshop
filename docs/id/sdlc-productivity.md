...
> **Durasi:** ~60 menit  
> **Tujuan:** Membangun alur kerja pengembang tingkat perusahaan dari instalasi pertama hingga rekayasa konteks, pengembangan berbasis spesifikasi dengan Conductor, dan pagar pengaman tata kelola.  
...
> **Apa yang terjadi:** Agen membaca `package.json`, memindai struktur direktori, dan memetakan arsitekturnya. Gemini CLI menjelajahi basis kode Anda sesuai permintaan — membaca file, mencari pola, dan melacak dependensi menggunakan alat-alat seperti `read_file`, `glob`, dan `grep_search` sesuai kebutuhan.
...
Ini menunjukkan setiap alat yang dapat digunakan oleh agen: operasi file, perintah shell, pencarian web, dan server MCP apa pun yang telah Anda konfigurasi.
...
> **Penemuan konteks JIT:** Agen hanya memuat file GEMINI.md yang relevan dengan file yang sedang dikerjakannya. Jika agen sedang mengedit `backend/controllers/productController.js`, agen memuat GEMINI.md proyek DAN GEMINI.md backend — tetapi bukan yang frontend.
...
- Aturan arsitektur (rute → controller → model)
- Anti-pola (tanpa callback, tanpa kredensial hardcode)
- Standar pengujian

### Menguji Penegakan Konteks

Minta agen untuk melanggar aturan dan lihat apakah agen mengoreksi dirinya sendiri:
...
> **Diharapkan:** Agen harus mengenali bahwa ini melanggar aturan GEMINI.md ("Tidak ada logika bisnis dalam file rute") dan sebagai gantinya membuat endpoint di dalam controller, dengan rute tipis yang mendelegasikan.
...
Kontrol apa yang dapat dan tidak dapat dilihat oleh agen:
...
> **Wawasan utama:** File-file ini sekarang menjadi sumber kebenaran untuk proyek Anda. File-file tersebut berupa Markdown, berada di repo Anda, di-commit dan ditinjau seperti kode lainnya. Saat Anda kembali besok — atau menyerahkan proyek ini kepada rekan kerja — AI akan melanjutkan tepat di titik terakhir Anda tinggalkan. Statusnya berada di dalam file, bukan di memori.
...
> **Lihatlah rencananya.** Rencana tersebut dibagi menjadi beberapa fase dengan tugas dan kotak centang tertentu. Fase 1: skema database. Fase 2: endpoint API. Fase 3: komponen frontend. Fase 4: pengujian. agen mengikuti rencana ini secara berurutan, mencentang tugas seiring berjalannya waktu.
...
> **Nilai enterprise:** Sub-agen `db-analyst` mendapatkan akses BigQuery hanya-baca. Sub-agen ini dapat melakukan kueri dan membuat daftar tabel, tetapi tidak akan pernah bisa menghapus data. Isolasi alat adalah tata kelola pada tingkat agen.

---
## 1.5 — Tata Kelola dan Mesin Kebijakan (10 mnt)
...
Contoh kebijakan:
- **Menolak** membaca `.env`, `.ssh`, dan file kredensial
...
Kebijakan diterapkan secara berjenjang berdasarkan urutan prioritas:

```
Default → Extension → Workspace → User → Admin (highest)
```

Kebijakan admin (ditetapkan pada tingkat sistem) mengesampingkan semua hal lainnya. Ini adalah cara perusahaan menerapkan pagar pengaman di seluruh organisasi.
...
> **Filosofi desain:** hook ini adalah penyuntik konteks yang ringan dan pengarah model — bukan pengeksekusi pengujian yang berat. Mereka menambahkan latensi total <200ms dan meningkatkan kualitas keputusan agen tanpa membebani sistem.
...
**Kebijakan tingkat admin** (diterapkan melalui MDM ke `/etc/gemini-cli/policies/`) menegakkan keamanan di seluruh organisasi yang tidak dapat dikesampingkan oleh pengembang individu:
...
**Kebijakan tingkat Workspace** (dimasukkan ke dalam repo Anda di `.gemini/policies/dev.toml`) menetapkan default tingkat tim:
...
> **Memeriksa kebijakan aktif:** Gunakan `/policies list` di CLI untuk melihat semua aturan yang mengatur sesi Anda, termasuk keputusan, tingkat prioritas, dan file sumbernya.
...
- **sandbox Docker**: Menjalankan perintah shell dalam wadah yang terisolasi
...
Menampilkan linimasa perubahan dalam sesi saat ini. Pilih titik untuk kembali.
...
---
## 1.7 — Agen Kustom untuk Seluruh SDLC (20 mnt)

> **Untuk pengguna mahir dan peserta yang kembali.** Bagian ini melampaui pembuatan kode untuk mencakup **seluruh siklus hidup pengembangan perangkat lunak** (SDLC) — ulasan, dokumentasi, kepatuhan, dan manajemen rilis. Setiap agen dapat digunakan secara independen. Anda dapat mulai dari titik mana pun.
...
- Sebuah **daftar izin alat** yang mengontrol apa saja yang dapat diakses oleh agen
- Sebuah **prompt sistem** yang mendefinisikan keahlian dan format outputnya

> **Prinsip desain utama:** Pisahkan pemikir dari pelaku. Agen hanya-baca untuk riset dan ulasan. Agen dengan akses tulis untuk implementasi. Jangan pernah mencampur investigasi dan mutasi dalam konteks yang sama.

Contoh-contoh di bawah ini menunjukkan bahwa Gemini CLI bukan sekadar pembuat kode — ini adalah **platform SDLC lengkap** yang mencakup ulasan, dokumentasi, kepatuhan, dan manajemen rilis.
...
> **Otomatiskan di CI/CD:** Untuk ulasan PR otomatis pada setiap pull request, gunakan GitHub Action resmi [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli). Instal dari CLI dengan `/setup-github` — ini akan mengonfigurasi file alur kerja, penangan pengiriman (dispatch handler), dan triase masalah secara otomatis. Lihat [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml) untuk contoh yang berfungsi.

---

### Agen 2: Penulis Dokumen

Menghasilkan dokumentasi API, README, dan komentar kode dari kode sumber. Hanya-baca — agen ini tidak akan pernah bisa memodifikasi file Anda.
...
Daripada membangun pemeriksa kepatuhan kustom, instal **[Ekstensi Keamanan](https://github.com/gemini-cli-extensions/security) resmi** — sebuah ekstensi yang dikelola Google dengan mesin SAST lengkap, pemindaian dependensi melalui [OSV-Scanner](https://github.com/google/osv-scanner), dan performa yang telah diuji (presisi 90%, perolehan (recall) 93% terhadap CVE nyata).
...
- Rahasia dan kunci API yang di-hardcode
...
Ini menggunakan [OSV-Scanner](https://github.com/google/osv-scanner) untuk mereferensikan silang dependensi Anda dengan [osv.dev](https://osv.dev), basis data kerentanan sumber terbuka milik Google.
...
> **Nilai loop luar:** Catatan rilis adalah salah satu tugas SDLC yang paling ditakuti. Agen ini membaca riwayat git DAN perubahan kode yang sebenarnya untuk menghasilkan catatan yang masuk akal bagi manajer produk.

---

### Menggabungkan Agen: Pipeline Lengkap

Kekuatan sebenarnya adalah menggabungkan agen ke dalam sebuah alur kerja. Setiap agen mendapatkan **konteks yang segar dan terfokus** — tidak ada satu agen pun yang mengakumulasi seluruh riwayat percakapan:
...
> **Mengapa ini berhasil:** Setiap langkah dimulai dengan konteks bersih yang terfokus pada pekerjaan spesifiknya. Investigator tidak membawa detail implementasi. Peninjau tidak membawa kebisingan investigasi. Ini adalah prinsip di balik setiap alur kerja AI berperforma tinggi.
...
Pada Bagian 1, Anda telah membangun blok-blok penyusun: sub-agen untuk peran khusus, Conductor untuk pengembangan berbasis spesifikasi, dan pemeriksa kepatuhan untuk penegakan kebijakan. Bagian 2 menunjukkan cara mempromosikan pola-pola ini ke dalam alur kerja loop luar.
...
Buat perubahan kode (tambahkan fitur, ubah pola arsitektur), lalu:
...
Pengembang baru menghabiskan waktu berhari-hari memetakan basis kode sebelum mereka dapat berkontribusi. Agen orientasi melakukan pemetaan ini dalam hitungan menit.
...
Pada Bagian 1, Anda menginstal [Ekstensi Keamanan](https://github.com/gemini-cli-extensions/security) untuk analisis lokal. Langkah selanjutnya adalah mempromosikannya ke CI — analisis keamanan otomatis pada setiap pull request.

#### Pola: Ekstensi Keamanan di GitHub Actions

Ekstensi Keamanan dilengkapi dengan alur kerja GitHub Actions yang siap digunakan. Salin secara langsung:
...
| Prompt Audit Ditulis Tangan | Ekstensi Keamanan |
|---|---|
...
> **Ini adalah pola CI dari slide 18** tetapi menggunakan ekstensi tingkat produksi yang telah diuji (presisi 90%, recall 93%) alih-alih prompt yang ditulis tangan. Perintah `/security:analyze` yang sama yang Anda jalankan secara lokal di §1.7 sekarang berjalan secara otomatis di setiap PR.
...
| Conductor spec-to-code (§1.4) | PRD → ADR → pipeline implementasi |
| mode headless (direferensikan dalam UC3) | Otomatisasi GitHub Action |
...
| **Agen kustom** | Agen khusus untuk ulasan, dokumen, catatan rilis — bukan hanya pengkodean |
...
| **Agen onboarding** | Pemetaan basis kode untuk pengembang baru — melacak jalur kode aktual |
...
→ Jelajahi ekosistem ekstensi: **[Ekosistem Ekstensi](extensions-ecosystem.md)** — penemuan, instalasi, pembuatan, dan pola enterprise

→ Untuk pengguna mahir: **[Pola Lanjutan](advanced-patterns.md)** — keterampilan prompting, loop verifikasi, rekayasa konteks, dan pengembangan paralel
