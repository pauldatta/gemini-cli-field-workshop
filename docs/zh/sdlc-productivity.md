...
> **时长：** 约 60 分钟  
> **目标：** 构建企业级开发者工作流，涵盖从首次安装、上下文工程、使用 Conductor 进行规范驱动的开发，到治理护栏的全过程。  
> **练习 PRD：** [产品愿望清单功能](https://github.com/pauldatta/gemini-cli-field-workshop/blob/main/exercises/prd_sdlc_productivity.md)
>
> *最后更新：2026-05-05 · [已针对 gemini-cli 仓库验证来源](https://github.com/google-gemini/gemini-cli)*
...
> **发生了什么：** 代理会读取 `package.json`，扫描目录结构，并映射架构。Gemini CLI 会按需探索你的代码库——根据需要使用 `read_file`、`glob` 和 `grep_search` 等工具来读取文件、搜索模式并追踪依赖关系。
...
这显示了代理可以使用的每一个工具：文件操作、shell 命令、网络搜索以及你配置的任何 MCP 服务器。
...
> **JIT 上下文发现：** 代理仅加载与其当前正在处理的文件相关的 GEMINI.md 文件。如果它正在编辑 `backend/controllers/productController.js`，它会加载项目级的 GEMINI.md 和后端（backend）的 GEMINI.md —— 但不会加载前端（frontend）的。
...
> **预期结果：** 代理应识别出这违反了 GEMINI.md 规则（“路由文件中不得包含业务逻辑”），并改为在控制器中创建端点，同时使用一个轻量级的路由进行委托。

> **强制执行规则：** `GEMINI.md` 提供了强有力的指导，但代理在复杂的重构过程中仍然可能会犯错。请将这些基于提示词的规则与接入 CI/CD 或 [Gemini CLI Hooks](https://www.geminicli.com/docs/hooks/) 的确定性代码检查工具（如 `dependency-cruiser`）结合使用。有关完整的环境设置，请参阅高级模式指南中的 [确定性强制执行](advanced-patterns.md#deterministic-enforcement)。
...
代理将使用 `write_file` 或 `edit` 直接更新你的 `GEMINI.md` 文件 —— 不需要斜杠命令。

> ⚠️ **注意：** 作为 Memory V2 更新的一部分，`/memory add` 已在 Gemini CLI v0.41.1 中被移除。底层的 `save_memory` 工具默认不再注册。请改用自然语言 —— 结果是相同的。有关详细信息，请参阅 [CHANGELOG.md](../../CHANGELOG.md) 和 [上游问题 #26563](https://github.com/google-gemini/gemini-cli/issues/26563)。

要让代理自动挖掘过去的会话并提出记忆更新供你审查，请在 `~/.gemini/settings.json` 中启用自动记忆（Auto Memory）：
...
然后使用 `/memory inbox` 在提取的事实被提交之前对其进行审查和批准。
...
> **为什么这很重要：** 如果没有 `.geminiignore`，代理可能会浪费上下文令牌去读取 `node_modules/`（数十万个文件）。有了它，代理将只关注你的源代码。

---
## 1.3 — Conductor：上下文优先的构建 (15 分钟)

### 为什么选择 Conductor？

计划模式非常适合一次性功能。但对于需要持久化规范、分阶段实施计划以及跨会话进度跟踪的多日项目——这就是 Conductor 的用武之地。
...
> **关键见解：** 这些文件现在是您项目的事实来源。它们是 Markdown 格式，存放在您的代码库中，像其他代码一样被提交和审查。当您明天回来——或者将这个项目移交给同事时——AI 会准确地从您离开的地方继续。状态保存在文件中，而不是在内存中。
...
> **如果您不同意该方法**——比如您想使用 GraphQL 而不是 REST——请直接编辑 `plan.md` 并重新运行。该计划是您和代理之间的契约。
...
> **按需探索：** 代理通过工具在您的代码库中导航——在实施计划的每个步骤时读取文件、追踪导入并交叉引用模式。像 `GEMINI.md` 和 Conductor 规范这样的上下文文件会与代理正在积极处理的文件一起加载。
...
扩展将技能、子代理、钩子、策略和 MCP 服务器打包成可安装的单元：
...
MCP（模型上下文协议）将 Gemini CLI 连接到外部数据源和工具：
...
- **允许** 实施者代理运行 `npm test` 和 `npm run lint`
- **默认** 其他所有操作为 `ask_user`（需要人工批准）
...
> **预期结果：** 代理应被策略引擎拦截。您将看到一条解释原因的拒绝消息。
...
管理员策略（在系统级别设置）会覆盖其他所有策略。这是企业强制执行全组织范围护栏的方式。

> **注意：** 工作区（Workspace）层目前在 CLI 源码中已被禁用。有关最新的层级状态，请参阅 [策略引擎参考](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)。
...
2. **BeforeTool → secret-scanner**：监控每次文件写入，检查是否有硬编码凭据
...
> **设计理念：** 这些钩子是轻量级的上下文注入器和模型引导器，而不是繁重的测试运行器。它们增加的总延迟小于 200 毫秒，并在不加重系统负担的情况下提高了代理的决策质量。

### 企业配置

对于全组织范围的工具限制，请使用带有管理员层级 TOML 策略的 [策略引擎](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)。有关实际操作演练，请参阅 [使用策略引擎保护 Gemini CLI](https://aipositive.substack.com/p/secure-gemini-cli-with-the-policy)。

**管理员层级策略**（通过 MDM 部署到 `/etc/gemini-cli/policies/`）强制执行全组织范围的安全措施，单个开发者无法覆盖：
...
**工作区层级策略**（签入到您的代码库 `.gemini/policies/dev.toml` 中）设置团队级别的默认值：
...
要强制执行企业身份验证，请在系统级别的 `settings.json` 中使用 `security.auth.enforcedType`（请参阅 [企业指南](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/enterprise.md)）。

### 沙盒

Gemini CLI 支持 [沙盒执行](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/sandbox.md)：
- **Docker 沙盒**：在隔离的容器中运行 shell 命令
- **macOS 沙盒**：使用 macOS 沙盒机制限制文件系统访问
...
显示可用的自定义命令。你可以在 `.gemini/commands/` 中定义自己的命令。
...
| **策略引擎** | TOML 中的代码化护栏 — deny、allow 或 ask_user |
...
> **面向高级用户和回归参与者。** 本节超越了代码生成，涵盖了**完整的软件开发生命周期 (SDLC)** —— 审查、文档、合规性和发布管理。每个代理都可以独立使用。您可以随时切入。
...
| 代理 | 目的 | 何时使用 |
|---|---|---|
| **`generalist`** | 拥有完整工具访问权限的通用代理 | 大批量或需要频繁交互的任务 |
| **`codebase_investigator`** | 架构映射与依赖分析 | “映射此应用中的身份验证流程” |
...
> **为什么这很重要：** 调查员 (investigator) 在只读模式下运行，具有集中的上下文。它在映射您的架构时不会意外修改文件。然后主代理使用该映射来规划实现。
...
- 一个**名称**，您可以使用 `@agent-name` 来调用
- 一个**描述**，CLI 用于自动路由
- 一个**工具允许列表**，控制代理可以访问的内容
- 一个**系统提示词**，定义其专业领域和输出格式

> **关键设计原则：** 将思考者与执行者分开。只读代理用于研究和审查。具有写入权限的代理用于实现。切勿在同一上下文中混合调查和修改操作。
...
> **在 CI/CD 中实现自动化：** 要在每个 pull request 上进行自动化的 PR 审查，请使用官方的 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action。通过 CLI 使用 `/setup-github` 安装它 —— 它会自动配置工作流文件、调度处理程序和问题分类。有关工作示例，请参见 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)。

---

### 代理 2：文档编写员

从源代码生成 API 文档、README 和代码注释。只读 —— 它绝不会修改您的文件。
...
> **外循环价值：** 这取代了数小时的手动文档工作。在每个冲刺 (sprint) 之后运行它以保持文档更新。

---

### 代理 3：安全分析（官方扩展）

无需构建自定义的合规性检查器，只需安装**官方 [Security Extension](https://github.com/gemini-cli-extensions/security)** —— 这是一个由 Google 维护的扩展，包含完整的 SAST 引擎、通过 [OSV-Scanner](https://github.com/google/osv-scanner) 进行的依赖项扫描，以及经过基准测试的性能（针对真实 CVE 的精确度为 90%，召回率为 93%）。
...
该扩展在您当前的分支差异上运行两遍 SAST 分析，检查以下内容：
- 硬编码的机密信息和 API 密钥
...
> **企业价值：** 此扩展附带用于 PoC 生成 (`poc`)、自动修补 (`security-patcher`) 和漏洞允许列表的技能。它开箱即用，可用于生产环境 —— 无需构建自定义合规代理。

---

### 代理 4：发行说明起草员

读取 git 历史记录和更改的文件，以生成结构化、对利益相关者友好的发行说明。
...
> **外循环价值：** 发行说明是最令人畏惧的 SDLC 任务之一。此代理读取 git 历史记录以及实际的代码更改，以生成对产品经理有意义的说明。

---

### 组合代理：完整流水线

真正的力量在于将代理组合成一个工作流。每个代理都会获得**新鲜、集中的上下文** —— 没有任何一个代理会积累完整的对话历史记录：
...
> **为什么这有效：** 每个步骤都以专注于其特定工作的干净上下文开始。调查员不携带实现细节。审查员不携带调查噪音。这是每个高性能 AI 工作流背后的原则。
...
> **时长：** 约 20 分钟（自定进度）
> **先决条件：** 完成上面的第 1 部分。熟悉自定义代理（§1.5）和 Conductor（§1.4）会有所帮助。

上面的练习集中在**内循环**——编写、测试和审查代码。但是代理也可以处理**外循环**——围绕代码的工作流：架构决策、开发人员入职、依赖审计和 CI 流水线自动化。

在第 1 部分中，您已经构建了基础模块：用于专门角色的子代理、用于规范驱动开发的 Conductor 以及用于策略执行的合规性检查器。第 2 部分展示了如何将这些模式推广到外循环工作流中。

---

### 2.1 — 结合子代理驱动开发的 ADR 生成器

架构决策记录（ADR）记录了做出某项技术选择的*原因*。手动编写它们非常繁琐，以至于团队往往完全跳过它们。借助来自 [superpowers 扩展](extensions-ecosystem.md#exercise-1-superpowers--methodology-as-extension)的子代理驱动开发（SDD）方法，您可以根据代码更改自动生成 ADR。
...
> **为什么这很重要：** ADR 是团队可以产出的最有价值的工件之一，也是最容易被忽视的工件之一。一个能从每个 PR 中生成 ADR 草稿的代理，将门槛从“编写文档”降低到了“审查文档”。采用这种模式的团队会自动建立起架构历史。

---

### 2.2 — 开发人员入职代理

新开发人员在能够做出贡献之前，需要花费数天时间来梳理代码库。入职代理可以在几分钟内完成这种梳理。
...
> **关键洞察：** 将此与阅读 README 并希望它是最新版本进行比较。代理追踪的是实际的代码路径，而不是可能已经偏离的文档。这是第 1 部分（§1.5）中的 `@codebase_investigator` 模式——但专门针对入职问题，并持久化为可重用的代理。

---

### 2.3 — CI 流水线中的安全分析

在第 1 部分中，您安装了 [安全扩展](https://github.com/gemini-cli-extensions/security) 进行本地分析。下一步是将其推广到 CI 中——在每个拉取请求上进行自动化安全分析。
...
3. 运行 `/security:scan-deps` 以检查依赖项漏洞
4. 将发现的问题作为 PR 评论发布
...
| 手写审计提示词 | 安全扩展 |
|---|---|
| 自由形式的提示词——每次运行结果不同 | 具有一致方法的结构化两遍 SAST 引擎 |
| 无漏洞分类 | 7 个类别，20 多种漏洞类型，严重程度标准（严重/高/中/低） |
| 无依赖项扫描 | 集成 OSV-Scanner，对照 Google 的漏洞数据库进行扫描 |
| 无修复工作流 | 内置 PoC 生成和自动修补技能 |
| 无白名单机制 | 用于已接受风险的持久化 `.gemini_security/vuln_allowlist.txt` |

> **这是幻灯片 18 中的 CI 模式**，但使用的是生产级、经过基准测试的扩展（90% 的精确率，93% 的召回率），而不是手写的提示词。您在 §1.7 中本地运行的同一个 `/security:analyze` 命令现在会在每个 PR 上自动运行。
...
| 安全扩展（§1.7） | CI 安全分析流水线 |
| Conductor 规范到代码（§1.4） | PRD → ADR → 实现流水线 |
| 无头模式（在 UC3 中引用） | GitHub Action 自动化 |

模式始终如一：**本地构建 → 验证 → 推广到 CI/CD → 在整个组织内扩展。** 帮助一名开发人员的代理变成了帮助整个团队的自动化工具。
...
| 功能 | 作用 |
|---|---|
| **GEMINI.md 层级结构** | 在各个层级编码项目约定 —— 代理会自动遵循它们 |
| **JIT 上下文发现** | 仅加载当前任务相关的上下文文件 |
| **记忆** | 跨会话持久化知识 |
| **Conductor** | 具有持久化计划和进度跟踪功能的规范驱动开发 |
...
| **安全扩展** | 官方 SAST + 依赖项扫描，具有 PoC 生成和自动修补功能 |
| **内置代理** | `generalist`、`codebase_investigator`、`cli_help` —— 无需设置即可委派 |
| **ADR 生成** | 子代理驱动，从 git diff 生成架构决策记录 |
| **入职代理** | 为新开发人员提供代码库映射 —— 追踪实际代码路径 |
