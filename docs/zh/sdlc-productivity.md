...
> **时长：** 约 60 分钟  
> **目标：** 构建一个企业级开发者工作流，涵盖从首次安装到上下文工程、使用 Conductor 进行规范驱动开发以及治理护栏的全过程。  
> **练习 PRD：** [产品愿望清单功能](https://github.com/pauldatta/gemini-cli-field-workshop/blob/main/exercises/prd_sdlc_productivity.md)
>
> *最后更新：2026-05-05 · [来源已对照 gemini-cli 仓库进行验证](https://github.com/google-gemini/gemini-cli)*
...
> **发生了什么：** 代理读取了 package.json，扫描了目录结构，并映射了架构。Gemini CLI 会按需探索你的代码库 —— 根据需要使用 `read_file`、`glob` 和 `grep_search` 等工具读取文件、搜索模式并追踪依赖关系。
...
> **JIT 上下文发现：** 代理仅加载与其当前正在处理的文件相关的 GEMINI.md 文件。如果它正在编辑 `backend/controllers/productController.js`，它会加载项目级别的 GEMINI.md 和后端的 GEMINI.md —— 但不会加载前端的 GEMINI.md。
...
> **预期结果：** 代理应识别出这违反了 GEMINI.md 规则（“路由文件中不得包含业务逻辑”），转而在控制器中创建端点，并使用一个轻量级的路由进行委托。
...
代理还可以使用 `save_memory` 工具自行保存记忆 —— 无论是在你明确要求它记住某事时，还是在你于 [settings.json](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/auto-memory.md) 中启用了 `experimental.autoMemory` 时自动保存。
...
> **为什么这很重要：** 如果没有 `.geminiignore`，代理可能会浪费上下文令牌去读取 `node_modules/`（数十万个文件）。有了它，代理只会专注于你的源代码。
...
计划模式非常适合一次性功能。但对于需要持久化规范、分阶段实施计划以及跨会话进度跟踪的多日项目来说——这就是 Conductor 的用武之地。
...
> **关键见解：** 这些文件现在是您项目的单一事实来源。它们是 Markdown 格式，存放在您的代码库中，像其他任何代码一样被提交和审查。当您明天回来——或者将这个项目移交给同事时——AI 会准确地从您上次停下的地方继续。状态保存在文件中，而不是内存中。
...
> **按需探索：** 代理通过工具导航您的代码库——在实施计划的每一步时读取文件、追踪导入并交叉引用模式。像 `GEMINI.md` 这样的上下文文件和 Conductor 规范会与代理正在积极处理的文件一起加载。
...
settings.json 包含一个 GitHub MCP 服务器。当配置了 `GITHUB_TOKEN` 后，代理可以：
- 读取仓库、issue 和 PR
- 创建 issue 和评论
- 发起拉取请求
...
1. **SessionStart → session-context**：在本次会话开始时注入了您的分支名称和脏文件计数
...
对于组织范围的工具限制，请使用带有管理员层级 TOML 策略的 [策略引擎](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)。有关实际操作演练，请参阅 [使用策略引擎保护 Gemini CLI](https://aipositive.substack.com/p/secure-gemini-cli-with-the-policy)。
...
**工作区层级策略**（检入到您的代码库 `.gemini/policies/dev.toml` 中）设置团队级别的默认值：
...
---
## 1.7 — 面向完整 SDLC 的自定义代理 (20 分钟)

> **面向高级用户和老学员。** 本节超越了代码生成，涵盖了**完整的软件开发生命周期**（SDLC）——代码审查、文档编写、合规性和发布管理。每个代理都可以独立使用。您可以随时切入。
...
| 代理 | 目的 | 何时使用 |
|---|---|---|
| **`generalist`** | 具有完整工具访问权限的通用代理 | 高容量或交互密集的任务 |
...
> **为什么这很重要：** 调查员（investigator）在具有集中上下文的只读模式下运行。它在映射您的架构时不会意外修改文件。然后主代理使用该映射来规划实现。
...
- 您使用 `@agent-name` 调用的**名称**
- CLI 用于自动路由的**描述**
- 控制代理可访问内容的**工具许可名单**
- 定义其专业知识和输出格式的**系统提示词**

> **关键设计原则：** 将思考者与执行者分开。只读代理用于研究和审查。具有写访问权限的代理用于实现。切勿在同一个上下文中混合调查和修改。

以下示例表明，Gemini CLI 不仅仅是一个代码生成器——它是一个涵盖代码审查、文档编写、合规性和发布管理的**完整 SDLC 平台**。
...
> **在 CI/CD 中实现自动化：** 要在每个拉取请求（PR）上进行自动 PR 审查，请使用官方的 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action。使用 `/setup-github` 从 CLI 安装它——它会自动配置工作流文件、调度处理程序和问题分类。有关有效示例，请参阅 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)。
...
与其构建自定义合规性检查器，不如安装**官方 [Security Extension](https://github.com/gemini-cli-extensions/security)**——这是一个由 Google 维护的扩展，具有完整的 SAST 引擎，通过 [OSV-Scanner](https://github.com/google/osv-scanner) 进行依赖项扫描，并具有基准性能（针对真实 CVE 的精确度为 90%，召回率为 93%）。
...
> **企业价值：** 此扩展附带用于 PoC 生成（`poc`）、自动修补（`security-patcher`）和漏洞许可名单的技能。它开箱即用，可用于生产环境——无需构建自定义合规性代理。

---

### 代理 4：发布说明起草员

读取 git 历史记录和更改的文件，以生成结构化、对利益相关者友好的发布说明。
...
> **外循环价值：** 发布说明是最令人头疼的 SDLC 任务之一。此代理读取 git 历史记录和实际的代码更改，以生成对产品经理有意义的说明。

---

### 组合代理：完整流水线

真正的强大之处在于将代理组合成一个工作流。每个代理都会获得**全新的、集中的上下文**——没有哪个代理会积累完整的对话历史记录：
...
> **为什么这行得通：** 每个步骤都从专注于其特定工作的干净上下文开始。调查员不携带实现细节。审查员不携带调查噪音。这是每个高性能 AI 工作流背后的原则。
...
上述练习主要集中在**内循环**——编写、测试和审查代码。但代理也可以处理**外循环**——围绕代码的工作流：架构决策、开发人员入职、依赖项审计和 CI 管道自动化。
...
> **为什么这很重要：** ADR 是团队可以产出的最有价值的工件之一，也是最容易被忽视的工件之一。一个能从每个 PR 生成 ADR 草稿的代理，将门槛从“编写文档”降低到了“审查文档”。采用这种模式的团队会自动建立起架构历史。
...
> **关键见解：** 将此与阅读 README 并希望它是最新版本进行比较。代理追踪的是实际的代码路径，而不是可能已经偏离的文档。这就是第 1 部分（§1.5）中的 `@codebase_investigator` 模式——但专门针对入职问题，并持久化为可重用的代理。

---

### 2.3 — CI 管道中的安全分析

在第 1 部分中，您安装了 [Security Extension](https://github.com/gemini-cli-extensions/security) 以进行本地分析。下一步是将其推广到 CI 中——在每个拉取请求上进行自动安全分析。
...
| 手写审计提示词 | 安全扩展 |
|---|---|
| 自由格式的提示词——每次运行结果各异 | 具有一致方法的结构化两遍 SAST 引擎 |
| 无漏洞分类 | 7 个类别，20 多种漏洞类型，严重性评估标准（严重/高/中/低） |
| 无依赖项扫描 | 针对 Google 漏洞数据库集成了 OSV-Scanner |
| 无修复工作流 | 内置 PoC 生成和自动修补技能 |
| 无白名单机制 | 用于已接受风险的持久化 `.gemini_security/vuln_allowlist.txt` |

> **这是幻灯片 18 中的 CI 模式**，但使用的是生产级、经过基准测试的扩展（90% 的精确率，93% 的召回率），而不是手写的提示词。您在 §1.7 中本地运行的相同 `/security:analyze` 命令现在会在每个 PR 上自动运行。
...
| 安全扩展（§1.7） | CI 安全分析管道 |
| Conductor 规范到代码（§1.4） | PRD → ADR → 实施管道 |
| 无头模式（在 UC3 中引用） | GitHub Action 自动化 |

模式始终如一：**本地构建 → 验证 → 推广到 CI/CD → 在整个组织内扩展。** 帮助一名开发人员的代理将成为帮助整个团队的自动化工具。
...
| 功能 | 它的作用 |
|---|---|
| **GEMINI.md 层级结构** | 在每个级别对项目约定进行编码 — 代理会自动遵循它们 |
| **JIT 上下文发现** | 仅加载与当前任务相关的上下文文件 |
| **记忆** | 在会话之间持久保存知识 |
| **Conductor** | 具有持久计划和进度跟踪的规范驱动开发 |
| **扩展** | 可安装的技能、代理、钩子和策略包 |
| **MCP 服务器** | 连接到外部工具（GitHub、BigQuery、Jira） |
...
| **ADR 生成** | 从 git 差异中生成由子代理驱动的架构决策记录 |
...
→ 继续前往 **[使用场景 2：遗留系统现代化](legacy-modernization.md)**
