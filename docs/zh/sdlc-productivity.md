...
> **时长：** 约 60 分钟  
> **目标：** 构建企业级开发者工作流，从首次安装开始，贯穿上下文工程、使用 Conductor 进行规范驱动开发，以及治理护栏。  
...
> **发生了什么：** 代理读取了 `package.json`，扫描了目录结构，并映射了架构。Gemini CLI 会按需探索你的代码库 —— 根据需要使用 `read_file`、`glob` 和 `grep_search` 等工具来读取文件、搜索模式并追踪依赖关系。
...
Gemini CLI 会在多个层级读取 `GEMINI.md` 文件，每个层级都会添加更具体的上下文：

![GEMINI.md 上下文层级](../assets/context-hierarchy.png)

> **JIT 上下文发现：** 代理仅加载与其当前正在处理的文件相关的 GEMINI.md 文件。如果它正在编辑 `backend/controllers/productController.js`，它会加载项目 GEMINI.md 和后端 GEMINI.md —— 但不会加载前端的 GEMINI.md。
...
此文件（在环境设置期间从 [`samples/gemini-md/project-gemini.md`](../../samples/gemini-md/project-gemini.md) 复制）定义了：
- 架构规则（路由 → 控制器 → 模型）
- 反模式（无回调，无硬编码凭据）
- 测试标准
...
> **预期结果：** 代理应识别出这违反了 GEMINI.md 规则（“路由文件中不能有业务逻辑”），并改为在控制器中创建端点，使用一个简单的路由进行委托。

> **强制执行规则：** 虽然 `GEMINI.md` 提供了强有力的指导（提示词工程），但 AI 在复杂的重构过程中仍可能偶尔产生幻觉或犯错。为了构建真正健壮的工作流，您应该使用 CI/CD 或 [Gemini CLI 钩子](https://geminicli.com/docs/hooks/) 将这些基于提示词的规则与确定性代码检查工具（如 `dependency-cruiser`）结合起来。有关如何进行设置，请参阅高级模式指南中的[确定性强制执行](advanced-patterns.md#deterministic-enforcement)。
...
代理还可以使用 `save_memory` 工具自行保存记忆 —— 无论是在您明确要求它记住某些内容时，还是在您在 [settings.json](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/auto-memory.md) 中启用 `experimental.autoMemory` 时自动保存。
...
> **为什么这很重要：** 如果没有 `.geminiignore`，代理可能会浪费上下文令牌去读取 `node_modules/`（数十万个文件）。有了它，代理将只关注您的源代码。
...
> **关键见解：** 这些文件现在是您项目的单一事实来源。它们是 Markdown 格式，存在于您的代码库中，像其他任何代码一样被提交和审查。当您明天回来——或者将这个项目交给同事时——AI 会准确地从您离开的地方继续。状态保存在文件中，而不是内存中。
...
> **按需探索：** 代理通过工具导航您的代码库——在实施计划的每一步时读取文件、追踪导入并交叉引用模式。像 `GEMINI.md` 和 Conductor 规范这样的上下文文件会与代理正在积极处理的文件一起加载。
...
- **允许**执行者代理运行 `npm test` 和 `npm run lint`
- **默认**将其他所有操作设置为 `ask_user`（需要人工批准）
...
管理员策略（在系统级别设置）会覆盖其他所有策略。这就是企业强制执行组织范围护栏的方式。

> **注意：** 工作区（Workspace）层目前在 CLI 源码中被禁用。请参阅 [策略引擎参考](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md) 获取最新的层级状态。
...
2. **BeforeTool → secret-scanner**：监控每次文件写入，检查是否有硬编码凭证
...
对于组织范围的工具限制，请使用带有管理员层级 TOML 策略的 [策略引擎](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)。有关实用的演练，请参阅 [使用策略引擎保护 Gemini CLI](https://aipositive.substack.com/p/secure-gemini-cli-with-the-policy)。

**管理员层级策略**（通过 MDM 部署到 `/etc/gemini-cli/policies/`）强制执行组织范围的安全措施，单个开发者无法覆盖：
...
对于企业身份验证强制执行，请在系统级别的 `settings.json` 中使用 `security.auth.enforcedType`（参阅 [企业指南](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/enterprise.md)）。
...
| **策略引擎** | TOML 中的护栏即代码 —— deny、allow 或 ask_user |
...
> **面向高级用户和老学员。** 本节不仅涉及代码生成，还将涵盖**完整的软件开发生命周期**（SDLC）——代码审查、文档编写、合规性检查以及发布管理。每个代理都可以独立使用。您可以随时从任何部分开始。
...
| 代理 | 目的 | 何时使用 |
|---|---|---|
| **`generalist`** | 拥有完整工具访问权限的通用代理 | 高容量或需要频繁交互的任务 |
...
> **为什么这很重要：** 调查员（investigator）在只读模式下运行，并具有集中的上下文。它在映射您的架构时不会意外修改文件。然后，主代理会使用该映射来规划实现。
...
- 一个您可以通过 `@agent-name` 调用的**名称**
- CLI 用于自动路由的**描述**
- 控制代理可以访问哪些内容的**工具许可名单**
- 定义其专业知识和输出格式的**系统提示词**

> **关键设计原则：** 将思考者与执行者分开。只读代理用于研究和审查。具有写入权限的代理用于实现。切勿在同一个上下文中混合调查和修改操作。

以下示例表明，Gemini CLI 不仅仅是一个代码生成器——它是一个**完整的 SDLC 平台**，涵盖了代码审查、文档编写、合规性检查和发布管理。

---

### 代理 1：PR 审查员

一个只读代理，用于审查代码更改的质量、错误和代码风格违规问题。
...
> **在 CI/CD 中实现自动化：** 要在每个拉取请求（pull request）上进行自动 PR 审查，请使用官方的 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action。通过 CLI 使用 `/setup-github` 安装它——它会自动配置工作流文件、调度处理程序和问题分类。有关工作示例，请参见 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)。
...
> **外循环价值：** 这取代了数小时的手动文档编写工作。在每个冲刺（sprint）之后运行它，以保持文档更新。

---

### 代理 3：安全分析（官方扩展）

与其构建自定义的合规性检查器，不如安装**官方的 [Security Extension](https://github.com/gemini-cli-extensions/security)**——这是一个由 Google 维护的扩展，包含完整的 SAST 引擎，通过 [OSV-Scanner](https://github.com/google/osv-scanner) 进行依赖项扫描，并具有经过基准测试的性能（针对真实 CVE 的精确度为 90%，召回率为 93%）。
...
该扩展对您当前分支的差异（diff）运行两遍 SAST 分析，检查以下内容：
- 硬编码的机密信息和 API 密钥
...
> **企业价值：** 此扩展附带了用于 PoC 生成（`poc`）、自动修补（`security-patcher`）和漏洞许可名单的技能。它开箱即用，已达到生产就绪状态——无需构建自定义的合规性代理。
...
> **外循环价值：** 发布说明是最令人畏惧的 SDLC 任务之一。该代理读取 git 历史记录和实际的代码更改，以生成对产品经理有意义的说明。

---

### 组合代理：完整流水线

真正的威力在于将代理组合成工作流。每个代理都会获得**全新的、集中的上下文**——没有任何一个代理会积累完整的对话历史记录：
...
> **为什么这行得通：** 每个步骤都以专注于其特定任务的干净上下文开始。调查员不会携带实现细节。审查员不会携带调查噪音。这是每个高性能 AI 工作流背后的原则。

---

### 深入了解

有关其他高级技术——提示词规范、验证循环、上下文工程以及并行开发——请参阅 **[高级模式](advanced-patterns.md)** 页面：
...
> **时长：** 约 20 分钟（自定进度）
> **先决条件：** 完成上述第 1 部分。熟悉自定义代理（§1.5）和 Conductor（§1.4）会有所帮助。

上述练习主要集中在**内循环**——编写、测试和审查代码。但代理也可以处理**外循环**——围绕代码的工作流：架构决策、开发者入职、依赖审计和 CI 管道自动化。

在第 1 部分中，您已经构建了基础模块：用于专门角色的子代理、用于规范驱动开发的 Conductor，以及用于策略执行的合规性检查器。第 2 部分将展示如何将这些模式推广到外循环工作流中。

---

### 2.1 — 基于子代理驱动开发的 ADR 生成器

架构决策记录（ADR）记录了做出某项技术选择的*原因*。手动编写它们非常繁琐，以至于团队通常会完全跳过这一步。借助来自 [superpowers 扩展](extensions-ecosystem.md#exercise-1-superpowers--methodology-as-extension)的子代理驱动开发（SDD）方法，您可以根据代码更改自动生成 ADR。
...
> **为什么这很重要：** ADR 是团队可以产出的最有价值的工件之一，但也是最容易被忽视的工件之一。一个能从每个 PR 生成 ADR 草案的代理，将门槛从“编写文档”降低到了“审查文档”。采用这种模式的团队会自动构建起架构历史记录。

---

### 2.2 — 开发者入职代理

新开发者在能够做出贡献之前，通常需要花费数天时间来理清代码库的结构。而入职代理可以在几分钟内完成这种梳理。
...
> **核心洞察：** 将此与阅读 README 并希望它是最新版本进行比较。代理追踪的是实际的代码路径，而不是可能已经偏离的文档。这是第 1 部分（§1.5）中的 `@codebase_investigator` 模式——但专门针对入职问题进行了定制，并持久化为可重用的代理。

---

### 2.3 — CI 管道中的安全分析

在第 1 部分中，您安装了 [Security Extension](https://github.com/gemini-cli-extensions/security) 进行本地分析。下一步是将其推广到 CI 中——在每个拉取请求上进行自动化的安全分析。

#### 模式：GitHub Actions 中的 Security Extension

Security Extension 附带了一个开箱即用的 GitHub Actions 工作流。直接复制它：
...
1. 将 Security Extension 安装到 CI 运行器中
2. 在 PR 差异上运行 `/security:analyze`
3. 运行 `/security:scan-deps` 检查依赖项漏洞
4. 将发现的问题作为 PR 评论发布

**为什么 Security Extension 优于手写提示词：**

| 手写审计提示词 | Security Extension |
|---|---|
| 自由格式的提示词——每次运行结果各异 | 具有一致方法论的结构化两遍 SAST 引擎 |
| 无漏洞分类 | 7 个类别，20 多种漏洞类型，严重性分级（严重/高/中/低） |
| 无依赖项扫描 | 集成了针对 Google 漏洞数据库的 OSV-Scanner |
| 无修复工作流 | 内置 PoC 生成和自动补丁技能 |
| 无白名单机制 | 用于已接受风险的持久化 `.gemini_security/vuln_allowlist.txt` |

> **这是幻灯片 18 中的 CI 模式**，但使用的是生产级、经过基准测试的扩展（90% 准确率，93% 召回率），而不是手写的提示词。您在 §1.7 中本地运行的同一个 `/security:analyze` 命令现在会在每个 PR 上自动运行。
...
| Security Extension（§1.7） | CI 安全分析管道 |
| Conductor 规范到代码（§1.4） | PRD → ADR → 实现管道 |
| 无头模式（在 UC3 中引用） | GitHub Action 自动化 |

模式始终如一：**本地构建 → 验证 → 推广到 CI/CD → 在整个组织内扩展。** 帮助一名开发者的代理，将变成帮助整个团队的自动化工具。
...
| **记忆** | 在会话之间持久化知识 |
| **Conductor** | 带有持久化计划和进度跟踪的规范驱动开发 |
| **扩展** | 包含技能、代理、钩子和策略的可安装包 |
| **MCP 服务器** | 连接到外部工具（GitHub、BigQuery、Jira） |
| **策略引擎** | TOML 中的护栏即代码 — 拒绝 (deny)、允许 (allow) 或询问用户 (ask_user) |
| **钩子** | 在代理生命周期事件中进行轻量级上下文注入和模型引导 |
| **沙盒** | 针对不受信任环境的隔离执行 |
| **自定义代理** | 用于审查、文档、发布说明的专用代理 — 不仅仅是编码 |
| **安全扩展** | 官方 SAST + 依赖项扫描，支持 PoC 生成和自动修补 |
| **内置代理** | `generalist`、`codebase_investigator`、`cli_help` — 无需环境设置即可委派 |
| **ADR 生成** | 由子代理驱动，从 git diff 生成架构决策记录 |
| **入职代理** | 为新开发人员提供代码库映射 — 追踪实际代码路径 |
| **CI 安全流水线** | GitHub Actions 中的安全扩展，用于自动漏洞分析 |
