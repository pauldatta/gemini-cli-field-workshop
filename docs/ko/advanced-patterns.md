...
AI 출력 품질을 개선하기 위해 할 수 있는 가장 큰 단일 개선 사항은 **질문하는 방법**을 바꾸는 것입니다.

### 문제점

대부분의 개발자는 단계별 지시를 내립니다:
...
| 에이전트가 이의를 제기하거나 대안을 제안할 수 없음 | 에이전트가 코드베이스에 가장 적합한 접근 방식을 선택함 |
| 검증 없음 — 수동으로 확인해야 함 | 성공 기준을 통한 내장된 검증 루프 |
| 하나의 경직된 경로 | 에이전트가 발견한 내용에 적응함 |

> **핵심 인사이트:** "무엇을 해야 할지 지시하지 말고, 성공 기준을 제시하고 어떻게 하는지 지켜보세요." 에이전트는 특정 목표를 달성할 때까지 루프를 반복하는 데 매우 뛰어납니다. 약한 기준("작동하게 만들어줘")은 지속적인 도움이 필요합니다. 강력한 기준은 독립적으로 실행될 수 있게 해줍니다.

### 연습 문제

ProShop을 사용하여 동일한 작업에 두 가지 접근 방식을 모두 시도해 보세요. 다음을 비교해 보세요:
1. 각각 몇 번의 턴이 소요되었나요?
2. 선언형 버전이 더 나은 접근 방식을 찾았나요?
3. 어느 쪽이 더 깔끔한 코드를 생성했나요?

---
## 컨텍스트 규율

에이전트의 컨텍스트 윈도우에 있는 모든 토큰은 다음 응답의 집중도를 약간 떨어뜨립니다. 컨텍스트는 예산입니다. 제한된 기기의 메모리처럼 관리하세요.
...
이렇게 하면 GEMINI.md, 메모리, 파일 상태는 그대로 유지하면서 대화형 컨텍스트를 초기화합니다. 에이전트는 프로젝트 지식을 모두 유지한 채로 새롭게 다시 시작합니다.
...
명시적인 성공 기준과 완료 약속이 포함된 리팩터링 작업을 에이전트에게 부여하세요. 에이전트가 테스트 통과(green)에 도달할 때까지 테스트 실패를 반복하며 수정하는 과정을 지켜보세요.
...
두 에이전트가 동시에 작업합니다. 서로 상대방의 변경 사항을 보지 못합니다. PR을 통해 두 작업 모두 병합하세요.
...
> **Scion 사용 시기:** 5개 이상의 동시 에이전트 작업을 수행하는 팀, 에이전트 간의 엄격한 격리가 필요한 프로젝트, 또는 여러 저장소에 걸쳐 AI 관리형 개발을 확장하려는 조직.

---
## 엔지니어링 헌장 패턴

에이전트에게 같은 내용을 두 번 말해야 한다면, 그 내용은 파일에 저장되어야 합니다.
...
1. ProShop을 위한 5가지 규칙이 포함된 GEMINI.md를 작성합니다.
2. 파일 **없이** 에이전트에게 기능 추가를 요청하고 — 출력을 확인합니다.
3. 파일과 **함께** 동일한 내용을 요청합니다.
4. 비교: 에이전트가 규칙을 따랐나요? 이전에 건너뛰었던 명확한 질문을 했나요?

---
## 스킬 기반 개발

스킬(Skill)은 시니어 엔지니어의 워크플로우를 에이전트에 직접 인코딩하는 구조화되고 재사용 가능한 지침 파일(`SKILL.md`)입니다. 단순한 프롬프트와 달리, 각 스킬에는 단계별 프로세스, 합리화 방지 테이블(에이전트가 단계를 건너뛰기 위해 사용할 수 있는 일반적인 변명과 문서화된 반박), 위험 신호(red flags) 및 검증 게이트가 포함되어 있습니다.
...
설치가 완료되면, 에이전트가 일치하는 작업을 인식할 때 스킬이 온디맨드로 활성화됩니다. UI를 구축 중인가요? `frontend-ui-engineering` 스킬이 자동으로 활성화됩니다. 테스트 실패를 디버깅 중인가요? `debugging-and-error-recovery`가 작동합니다.
...
| `/build` | 빌드 | 다음 태스크를 얇은 수직 슬라이스로 구현 |
...
둘 다 에이전트의 동작에 영향을 미치지만, 서로 다른 목적을 제공합니다:
...
2. `/spec` 실행 — "제품 비교" 기능에 대한 사양을 작성합니다.
3. `/build` 실행 — 첫 번째 슬라이스를 점진적으로 구현합니다.
4. `/test` 실행 — TDD 워크플로우가 red-green-refactor를 강제하는지 확인합니다.
5. 비교: 구조화된 워크플로우가 단순한 "비교 기능 추가" 프롬프트와 어떻게 다른가요?

---
## Google 관리형 MCP 서버

Google은 에이전트가 Google Cloud 서비스, Workspace 앱 및 개발자 도구에 직접적이고 통제된 방식으로 액세스할 수 있도록 **50개 이상의 관리형 MCP 서버**를 제공합니다. 로컬 서버 설치가 필요하지 않습니다.

### 관리형 MCP를 사용하는 이유

| 고려 사항 | 관리형 MCP의 해결 방법 |
|---|---|
| **보안** | 도구 수준 액세스 제어를 위한 IAM Deny 정책, 프롬프트 인젝션 방어를 위한 Model Armor |
...
[Developer Knowledge MCP 서버](https://developers.google.com/knowledge/mcp)는 에이전트가 Firebase, Cloud, Android, Maps 등 공식 Google 문서에 기반하여 작동하도록 합니다. 에이전트는 API 서명을 환각(hallucinate)하는 대신, 실시간 문서 말뭉치를 쿼리합니다.
...
> **거버넌스:** [IAM Deny 정책](https://docs.cloud.google.com/mcp/control-mcp-use-iam#deny-all-mcp-tool-use)을 사용하여 에이전트가 호출할 수 있는 MCP 도구를 제한하세요. 간접적인 프롬프트 인젝션 및 데이터 유출을 방어하려면 [Model Armor](https://docs.cloud.google.com/model-armor/model-armor-mcp-google-cloud-integration)와 결합하세요.

### 연습 문제

1. Google Cloud 프로젝트에서 Developer Knowledge API 키를 가져옵니다.
2. 위의 한 줄 설치 명령어를 사용하여 Gemini CLI 구성에 Developer Knowledge MCP 서버를 추가합니다.
3. 에이전트에게 질문합니다: *"사용자 지정 도메인으로 Cloud Run 서비스를 어떻게 배포하나요?"*
4. 확인: 응답이 공식 문서를 인용하나요? MCP 서버가 연결되지 않았을 때의 답변과 비교해 보세요.
...
> **사전 요구 사항:** Python 3.11+, [uv](https://docs.astral.sh/uv/getting-started/installation/) 및 Node.js. 환경에 대한 참고 사항은 `setup.sh`를 참조하세요.
...
| `agents-cli eval run` | 에이전트 평가 실행 (심사위원으로서의 LLM(LLM-as-judge), 궤적 채점) |
...
| `google-agents-cli-eval` | 평가 방법론 — 지표, 평가 세트(evalsets), 심사위원으로서의 LLM(LLM-as-judge) |
...
5. 설치된 스킬이 어떻게 Gemini CLI를 안내하여 원래라면 알지 못했을 ADK 전용 패턴을 사용하게 하는지 관찰합니다.
...
→ 핵심 기능을 확인하려면 **[사용 사례 1: SDLC 생산성 향상](sdlc-productivity.md)**으로 돌아가기
