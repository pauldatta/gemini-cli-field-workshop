...
---
## 프롬프트 작성 기법: 목표 vs. 지시사항

AI 출력 품질을 개선할 수 있는 가장 큰 단일 개선 사항은 **질문하는 방법**을 바꾸는 것입니다.
...
이는 더 나은 접근 방식이 존재하더라도 에이전트가 특정 경로를 따르도록 강제합니다. 에이전트는 반론을 제기하거나, 트레이드오프를 드러내거나, 적응할 수 없습니다.
...
> **핵심 통찰:** "무엇을 해야 할지 지시하지 말고, 성공 기준을 제시하고 진행되는 것을 지켜보세요." 에이전트는 특정 목표를 달성할 때까지 반복하는 데 탁월합니다. 약한 기준("작동하게 만들기")은 지속적인 도움이 필요합니다. 강력한 기준은 에이전트가 독립적으로 실행되도록 합니다.

### 연습 문제

ProShop에서 동일한 작업에 대해 두 가지 접근 방식을 모두 시도해 보세요. 다음을 비교해 보세요:
1. 각각 몇 번의 턴이 소요되었나요?
...
이렇게 하면 GEMINI.md, 메모리 및 파일 상태는 그대로 유지하면서 대화 컨텍스트를 초기화합니다. 에이전트는 프로젝트 지식을 모두 유지한 채로 새롭게 다시 시작합니다.
...
메모리는 세션 및 `/clear` 초기화 후에도 유지됩니다. 초기화하기 전에 중요한 발견 사항을 저장하세요.
...
또는 영구적인 컨텍스트를 위해 GEMINI.md에 가져오기(import)로 추가하세요:
...
> 가져오기 구문은 [GEMINI.md 참조](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/gemini-md.md)를 확인하세요.

#### 4. 에이전트 위임을 통한 격리

각 사용자 지정 에이전트는 고유한 컨텍스트 윈도우를 갖습니다. 이를 전략적으로 사용하세요:
...
1. 세션을 시작하고 세 가지 기능을 순차적으로 구축합니다(의도적으로 컨텍스트 축적).
2. 15-20턴 부근에서 품질 저하를 관찰합니다.
3. `/memory add`를 실행하여 주요 사실을 저장합니다.
4. `/clear`를 실행합니다 — 즉각적인 품질 향상을 관찰합니다.
5. 에이전트에게 중단된 부분부터 계속하라고 요청합니다 — 메모리 + 파일 상태를 통해 이어서 작업합니다.
...
에이전트에게 명시적인 성공 기준과 완료 약속이 포함된 리팩토링 작업을 부여해 보세요. 테스트 실패를 반복하며 성공(green)에 도달할 때까지 지켜보세요.

---
## 워크트리를 활용한 병렬 개발

서로 다른 브랜치에서 여러 Gemini CLI 세션을 동시에 실행하세요. 각각은 완전히 격리된 상태로 실행됩니다.

### 문제점

한 번에 하나의 브랜치만 체크아웃할 수 있습니다. 별도의 에이전트를 사용하여 기능 개발, 버그 수정, 리팩터링을 동시에 작업하려는 경우 충돌이 발생합니다.
...
| **검토** | 각 에이전트가 자신의 워크트리 내 브랜치에 커밋 |
| **통합** | PR을 통해 브랜치를 `main`으로 다시 병합 |
...
두 에이전트가 동시에 작업합니다. 어느 쪽도 상대방의 변경 사항을 볼 수 없습니다. PR을 통해 두 가지 모두 병합하세요.

---
## 멀티 에이전트 오케스트레이션

프로젝트 전반에 걸쳐 수십 개의 에이전트를 관리하는 팀을 위해 오케스트레이션 도구는 엔터프라이즈급 격리, 관찰 가능성 및 확장성을 제공합니다.
...
| **Grove** | 프로젝트 네임스페이스 — 일반적으로 git 리포지토리와 1:1로 매칭됨 |
| **템플릿** | 에이전트 청사진: 시스템 프롬프트 + 스킬 + 도구 권한 |
| **런타임** | Docker, Podman, Apple Container 또는 Kubernetes |

> **Scion 사용 시기:** 5개 이상의 동시 에이전트 작업을 수행하는 팀, 에이전트 간의 엄격한 격리가 필요한 프로젝트, 또는 여러 리포지토리에 걸쳐 AI 관리 개발을 확장하려는 조직.
...
3. 파일이 **있는** 상태에서 동일한 요청하기
4. 비교: 에이전트가 규칙을 따랐나요? 이전에 건너뛰었던 명확화를 위한 질문을 했나요?

---
## 결정론적 강제 적용

엔지니어링 헌장(`GEMINI.md`)은 에이전트를 *안내*하는 데(프롬프트 엔지니어링) 탁월하지만, 100% 준수를 보장할 수는 없습니다. 에이전트도 사람과 마찬가지로 복잡한 리팩터링 과정에서 실수를 하거나 잘못된 패턴을 환각(hallucinate)할 수 있습니다(이러한 현상을 *프롬프트 드리프트(Prompt Drift)*라고 합니다).

견고한 SDLC를 구축하려면 AI 생성 기능과 **가드레일(Guardrails)**을 결합해야 합니다. 가드레일은 AI가 보고, 수행하고, 생성할 수 있는 것을 제한하는 결정론적 경계입니다.

### 입력 vs 출력 가드레일

엔터프라이즈 SDLC에서 가드레일은 두 가지 범주로 나뉩니다.

1. **입력 가드레일 (생성 전):** 악의적인 입력으로부터 에이전트를 보호하거나 컨텍스트를 제한합니다.
   - *예시:* `.geminiignore` 파일은 에이전트가 불필요한 파일을 읽지 못하게 합니다.
   - *예시:* `GEMINI.md`는 아키텍처에 대한 기대치를 사전에 설정합니다.
2. **출력 가드레일 (생성 후):** 생성 *후* 병합되거나 배포되기 *전*에 에이전트의 출력을 검증합니다.
   - *예시:* 결정론적 린터(linter)를 사용하여 아키텍처 경계를 강제합니다.
   - *예시:* 테스트 스위트나 스캐너를 실행하여 유출된 비밀 정보를 탐지합니다.

### 시너지: "AI가 제안하고, CI가 처리한다"

LLM이 자체적으로 아키텍처를 감시하는 데만 의존하는 대신, 기존의 소프트웨어 엔지니어링 도구(출력 가드레일)를 사용하여 규칙을 강제 적용하세요.

1. **가이드 (`GEMINI.md`):** 에이전트에게 처음부터 코드를 올바르게 작성하는 *방법*을 알려줍니다(입력).
2. **가드 (린터/정적 분석):** 에이전트가 실수를 할 경우 결정론적으로 이를 잡아냅니다(출력).
3. **루프:** 가드 도구가 실패하면, 오류 출력이 에이전트에게 피드백되며([Gemini CLI 훅](https://geminicli.com/docs/hooks/)을 사용한 지속적인 검증 루프를 통해), 에이전트는 이 명확한 피드백을 바탕으로 자신의 실수를 자동으로 수정합니다.

### 실제 강제 적용

0이 아닌 코드(non-zero code)로 종료될 수 있는 모든 결정론적 도구는 강제 적용 도구(enforcer)로 작동할 수 있습니다. 이러한 도구를 CI/CD 파이프라인에서 실행하거나, git `pre-commit` 훅으로 실행하거나, Gemini CLI의 `AfterAgent` 훅을 통해 직접 실행하도록 설정할 수 있습니다.

**결정론적 강제 적용 도구의 예시:**
- **표준 린터:** 라우트 파일의 코드 복잡도 제한(예: `max-lines-per-function`)을 강제하는 ESLint 또는 Ruff.
- **보안 스캐너:** 에이전트가 실수로 API 키를 하드코딩하지 않았는지 확인하는 `gitleaks`와 같은 도구.
- **아키텍처 린터:** 종속성 그래프를 파싱하여 계층 경계를 강제하는 도구.
...
에이전트의 워크플로우 내에서 이를 자동화하려면, Gemini CLI가 이해할 수 있도록 린터의 출력을 JSON 형식으로 포맷해야 합니다. 먼저, 린터를 실행하고 오류를 캡처하는 훅 스크립트를 생성합니다.
...
그런 다음, 이 스크립트를 설정에서 `AfterAgent` 훅으로 등록합니다.
...
이제 에이전트가 잘못된 임포트(import)를 생성하면, 훅이 즉시 린터 오류를 대화에 다시 피드백하여 에이전트가 위반 사항을 해결하도록 강제합니다.

### 연습 문제
1. 프로젝트에서 데이터베이스 모델을 직접 임포트하는 라우트 파일을 생성합니다.
2. 이 패턴을 차단하도록 결정론적 강제 적용 도구(`dependency-cruiser` 또는 사용자 지정 ESLint 규칙 등)를 구성합니다.
3. 에이전트에게 "라우트에 새 엔드포인트를 추가해 줘"라고 요청하고, 잘못된 패턴을 복사하는지 아니면 수정하는지 관찰합니다.
4. 강제 적용 도구를 실행하고, 오류를 에이전트에게 피드백한 다음, 위반 사항을 해결하도록 요청합니다.

---
## 스킬 기반 개발

스킬(Skill)은 시니어 엔지니어의 워크플로우를 에이전트에 직접 인코딩하는 구조화되고 재사용 가능한 지침 파일(`SKILL.md`)입니다. 단순한 프롬프트와 달리, 각 스킬에는 단계별 프로세스, 합리화 방지 테이블(에이전트가 단계를 건너뛰기 위해 사용할 수 있는 일반적인 변명과 문서화된 반박), 적신호(red flags), 그리고 검증 게이트가 포함되어 있습니다.

### 스킬이 단순한 프롬프트보다 나은 이유

| 단순한 프롬프트 | 구조화된 스킬 |
|---|---|
| "이것에 대한 테스트 작성해 줘" | 테스트 피라미드 목표(80/15/5)와 함께 Red-Green-Refactor 워크플로우 활성화 |
| "이 코드 리뷰해 줘" | 심각도 레이블(Nit/Optional/FYI) 및 변경 규모 기준을 포함한 5축 리뷰 실행 |
| "이것을 안전하게 만들어 줘" | 3계층 경계 시스템과 함께 OWASP Top 10 체크리스트 트리거 |
| 중지 기준 없음 | 내장된 검증 게이트 — 에이전트는 다음으로 넘어가기 전에 증거를 제시해야 함 |

### 커뮤니티 스킬 설치

[agent-skills](https://github.com/addyosmani/agent-skills) 팩은 전체 SDLC를 다루는 20개의 프로덕션 수준 스킬을 제공합니다. 다음 명령어 하나로 설치할 수 있습니다:
...
둘 다 에이전트의 동작에 영향을 미치지만, 서로 다른 목적을 수행합니다:

| | 스킬 | GEMINI.md |
|---|---|---|
| **로드 시점** | 작업이 일치할 때 온디맨드로 | 모든 프롬프트에 항상 |
| **토큰 비용** | 활성화될 때까지 최소화됨 | 지속적인 오버헤드 |
| **최적의 용도** | 단계별 워크플로우 (TDD, 보안 리뷰, 배포) | 항상 켜져 있는 프로젝트 규칙 (기술 스택, 코딩 표준) |

**경험 법칙:** *모든* 프롬프트에서 활성화되기를 원한다면 GEMINI.md에 넣으세요. 특정 단계에만 해당한다면 스킬로 설치하세요.

### 연습 문제

1. ProShop 워크스페이스에 agent-skills 팩을 설치합니다.
2. `/spec`을 실행합니다 — "제품 비교" 기능에 대한 사양을 작성합니다.
3. `/build`를 실행합니다 — 첫 번째 슬라이스를 점진적으로 구현합니다.
4. `/test`를 실행합니다 — TDD 워크플로우가 red-green-refactor를 강제하는지 확인합니다.
5. 비교: 구조화된 워크플로우가 단순한 "비교 기능 추가해 줘" 프롬프트와 어떻게 다른가요?

---
## Google 관리형 MCP 서버

Google은 에이전트가 로컬 서버 설치 없이 Google Cloud 서비스, Workspace 앱 및 개발자 도구에 직접적이고 관리되는 액세스를 할 수 있도록 **50개 이상의 관리형 MCP 서버**를 제공합니다.
...
| **검색** | Agent Registry — MCP 서버를 찾고 관리하기 위한 통합 디렉토리 |
| **관측성** | 전체 작업 포렌식을 위한 OTel Tracing + Cloud Audit Logs |
| **상호 운용성** | Gemini CLI, Claude Code, Cursor, VS Code, LangChain, ADK, CrewAI와 연동 |

### Developer Knowledge MCP

[Developer Knowledge MCP 서버](https://developers.google.com/knowledge/mcp)는 에이전트가 Firebase, Cloud, Android, Maps 등 공식 Google 문서에 기반하도록 합니다. 에이전트는 API 서명을 환각(hallucinate)하는 대신 실시간 문서 말뭉치를 쿼리합니다.
...
| `answer_query` | 문서 말뭉치에서 합성되고 근거 있는 답변 가져오기 |
...
> **거버넌스:** [IAM 거부 정책](https://docs.cloud.google.com/mcp/control-mcp-use-iam#deny-all-mcp-tool-use)을 사용하여 에이전트가 호출할 수 있는 MCP 도구를 제한하세요. [Model Armor](https://docs.cloud.google.com/model-armor/model-armor-mcp-google-cloud-integration)와 결합하여 간접적인 프롬프트 인젝션 및 데이터 유출을 방어하세요.
...
| `google-agents-cli-deploy` | 배포 — Agent Runtime, Cloud Run, GKE, CI/CD, 보안 비밀 |
...
5. 설치된 스킬이 어떻게 Gemini CLI를 안내하여, 그렇지 않았다면 알지 못했을 ADK 전용 패턴을 사용하게 하는지 관찰합니다.
...
| [pauldatta/gemini-cli-field-workshop](https://github.com/pauldatta/gemini-cli-field-workshop) | 이 워크숍의 소스 저장소 |
| [Gemini CLI Docs](https://geminicli.com) | 공식 문서 |

---
## 다음 단계

→ 핵심 기능을 알아보려면 **[사용 사례 1: SDLC 생산성 향상](sdlc-productivity.md)**으로 돌아가기

→ 브라운필드 워크플로우를 알아보려면 **[사용 사례 2: 레거시 코드 현대화](legacy-modernization.md)**로 계속 진행하기
