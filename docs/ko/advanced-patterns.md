...
> **소요 시간:** 약 45분 (자기 주도)  
> **목표:** 프롬프트 규율, 검증 루프, 컨텍스트 엔지니어링 및 병렬 개발을 마스터합니다. 이러한 기술은 모든 Gemini CLI 워크플로우에서 작동합니다.  
...
> **핵심 통찰:** "무엇을 해야 할지 지시하지 말고, 성공 기준을 제시하고 지켜보세요." 에이전트는 특정 목표를 달성할 때까지 루프를 도는 데 매우 뛰어납니다. 약한 기준("작동하게 만들기")은 지속적인 도움이 필요합니다. 강력한 기준은 에이전트가 독립적으로 실행되도록 합니다.
...
에이전트의 컨텍스트 윈도우에 있는 모든 토큰은 다음 응답의 초점을 약간 흐리게 만듭니다. 컨텍스트는 예산과 같습니다. 제약이 있는 기기의 메모리처럼 관리하세요.

### 컨텍스트 과부하의 증상

- 에이전트가 같은 말을 반복하기 시작함
- 환각 현상 증가 (존재하지 않는 파일 참조)
...
이는 GEMINI.md, 메모리 및 파일 상태는 그대로 유지하면서 대화 컨텍스트를 초기화합니다. 에이전트는 프로젝트에 대한 모든 지식을 유지한 채로 새롭게 다시 시작합니다.
...
대규모 사양(spec)을 대화에서 빼내어 파일로 이동하세요:
...
1. 세션을 시작하고 세 가지 기능을 순차적으로 구축합니다 (의도적으로 컨텍스트 누적).
2. 15-20턴 부근에서 품질 저하를 관찰합니다.
3. `/memory add`를 실행하여 핵심 사실을 저장합니다.
4. `/clear`를 실행하고 즉각적인 품질 향상을 관찰합니다.
5. 에이전트에게 중단된 부분부터 계속하라고 요청합니다 — 메모리와 파일 상태를 통해 작업을 재개합니다.
...
3. **반복적인 복구** — 테스트가 실패하면 사용자에게 묻는 대신 스스로 수정하고 다시 실행합니다.
...
에이전트에게 명시적인 성공 기준과 완료 약속이 포함된 리팩토링 작업을 부여하세요. 테스트 실패를 반복하며 성공(green)에 도달할 때까지 지켜보세요.
...
| **검토** | 각 에이전트가 자신의 워크트리 내에 있는 브랜치에 커밋 |
...
두 개의 터미널 창을 엽니다. 워크트리를 사용하여 다음을 수행합니다:
1. 한 창에서는 위시리스트 기능 추가
2. 다른 창에서는 장바구니 총액 계산 오류 수정

두 에이전트가 동시에 작업합니다. 서로 상대방의 변경 사항을 볼 수 없습니다. PR을 통해 둘 다 병합합니다.

---
## 다중 에이전트 오케스트레이션

프로젝트 전반에 걸쳐 수십 개의 에이전트를 관리하는 팀을 위해, 오케스트레이션 도구는 엔터프라이즈급 격리, 관찰 가능성 및 확장을 제공합니다.

### Scion (Google Cloud Platform)

**[Scion](https://github.com/GoogleCloudPlatform/scion)**은 에이전트를 격리된 동시 프로세스(각각 자체 컨테이너에 있음)로 실행하는 실험적인 다중 에이전트 오케스트레이터입니다.
...
| **Grove** | 프로젝트 네임스페이스 — 일반적으로 git 리포지토리와 1:1로 매핑됨 |
| **템플릿** | 에이전트 청사진: 시스템 프롬프트 + 스킬 + 도구 권한 |
| **런타임** | Docker, Podman, Apple Container 또는 Kubernetes |

> **Scion 사용 시기:** 5개 이상의 동시 에이전트 작업을 수행하는 팀, 에이전트 간의 엄격한 격리가 필요한 프로젝트, 또는 여러 리포지토리에 걸쳐 AI 관리 개발을 확장하는 조직.
...
스킬은 시니어 엔지니어의 워크플로우를 에이전트에 직접 인코딩하는 구조화되고 재사용 가능한 지침 파일(`SKILL.md`)입니다. 원시 프롬프트와 달리 각 스킬에는 단계별 프로세스, 합리화 방지 테이블(에이전트가 단계를 건너뛰기 위해 사용할 수 있는 일반적인 변명과 문서화된 반박), 위험 신호(red flags) 및 검증 게이트가 포함되어 있습니다.

### 스킬이 원시 프롬프트보다 나은 이유

| 원시 프롬프트 | 구조화된 스킬 |
|---|---|
| "이것에 대한 테스트 작성" | 테스트 피라미드 목표(80/15/5)와 함께 Red-Green-Refactor 워크플로우 활성화 |
| "이 코드 리뷰" | 심각도 레이블(Nit/Optional/FYI) 및 변경 크기 기준을 포함한 5축 리뷰 실행 |
| "이것을 안전하게 만들기" | 3계층 경계 시스템과 함께 OWASP Top 10 체크리스트 트리거 |
| 중지 기준 없음 | 내장된 검증 게이트 — 에이전트는 다음으로 넘어가기 전에 증거를 제시해야 함 |

### 커뮤니티 스킬 설치

[agent-skills](https://github.com/addyosmani/agent-skills) 팩은 전체 SDLC를 다루는 20개의 프로덕션 등급 스킬을 제공합니다. 다음 명령어 하나로 설치하세요:
...
| `/ship` | 배포 | 병렬 페르소나 팬아웃을 통한 출시 전 체크리스트 |
...
1. ProShop 작업 공간에 agent-skills 팩 설치
2. `/spec` 실행 — "제품 비교" 기능에 대한 사양 작성
3. `/build` 실행 — 첫 번째 슬라이스를 점진적으로 구현
4. `/test` 실행 — TDD 워크플로우가 red-green-refactor를 강제하는지 확인
5. 비교: 구조화된 워크플로우가 원시적인 "비교 기능 추가" 프롬프트와 어떻게 다른가요?
...
|---|---|
| **보안** | 도구 수준의 액세스 제어를 위한 IAM Deny 정책; 프롬프트 인젝션 방어를 위한 Model Armor |
...
[Developer Knowledge MCP 서버](https://developers.google.com/knowledge/mcp)는 에이전트가 Firebase, Cloud, Android, Maps 등 공식 Google 문서에 기반하도록 합니다. 에이전트는 API 서명을 환각(hallucinating)하는 대신 실시간 문서 코퍼스를 쿼리합니다.
...
> **거버넌스:** 에이전트가 호출할 수 있는 MCP 도구를 제한하려면 [IAM Deny 정책](https://docs.cloud.google.com/mcp/control-mcp-use-iam#deny-all-mcp-tool-use)을 사용하세요. 간접적인 프롬프트 인젝션 및 데이터 유출을 방어하려면 [Model Armor](https://docs.cloud.google.com/model-armor/model-armor-mcp-google-cloud-integration)와 결합하세요.

### 실습

1. Google Cloud 프로젝트에서 Developer Knowledge API 키를 가져옵니다.
2. 위의 원라이너를 사용하여 Gemini CLI 구성에 Developer Knowledge MCP 서버를 추가합니다.
...
→ 핵심 기능을 확인하려면 **[사용 사례 1: SDLC 생산성 향상](sdlc-productivity.md)**으로 돌아가세요.

→ 브라운필드 워크플로우를 확인하려면 **[사용 사례 2: 레거시 코드 현대화](legacy-modernization.md)**로 계속 진행하세요.
