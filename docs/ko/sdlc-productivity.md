...
> **소요 시간:** 약 60분  
> **목표:** 초기 설치부터 컨텍스트 엔지니어링, Conductor를 활용한 스펙 기반 개발, 거버넌스 가드레일에 이르기까지 엔터프라이즈급 개발자 워크플로우 구축.  
...
여기에는 파일 작업, 셸 명령, 웹 검색 및 구성한 모든 MCP 서버 등 에이전트가 사용할 수 있는 모든 도구가 표시됩니다.
...
> **규칙 강제 적용:** `GEMINI.md`는 강력한 지침을 제공하지만, 에이전트는 복잡한 리팩토링 중에 여전히 실수를 할 수 있습니다. 이러한 프롬프트 기반 규칙을 CI/CD 또는 [Gemini CLI Hooks](https://www.geminicli.com/docs/hooks/)에 연결된 결정론적 린터(`dependency-cruiser` 등)와 결합하세요. 전체 설정은 고급 패턴 가이드의 [결정론적 강제 적용](advanced-patterns.md#deterministic-enforcement)을 참조하세요.
...
에이전트는 `save_memory` 도구를 사용하여 스스로 메모리를 저장할 수도 있습니다. 사용자가 명시적으로 무언가를 기억하라고 요청할 때나, [settings.json](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/auto-memory.md)에서 `experimental.autoMemory`를 활성화한 경우 자동으로 저장합니다.
...
플랜 모드는 일회성 기능에 적합합니다. 하지만 영구적인 사양, 단계별 구현 계획, 여러 세션에 걸친 진행 상황 추적이 필요한 며칠짜리 프로젝트에는 Conductor가 필요합니다.
...
> **핵심 인사이트:** 이제 이 파일들은 프로젝트의 단일 진실 공급원(source of truth)이 됩니다. 이 파일들은 Markdown 형식이며, 리포지토리에 존재하고, 다른 코드와 마찬가지로 커밋되고 리뷰됩니다. 내일 다시 돌아오거나 이 프로젝트를 동료에게 넘길 때, AI는 여러분이 중단한 바로 그 지점부터 다시 시작합니다. 상태는 메모리가 아닌 파일에 저장됩니다.
...
> **계획을 살펴보세요.** 특정 작업과 체크박스가 있는 단계로 나뉘어 있습니다. 1단계: 데이터베이스 스키마. 2단계: API 엔드포인트. 3단계: 프론트엔드 컴포넌트. 4단계: 테스트. 에이전트는 이 계획을 순서대로 따르며 진행하면서 작업을 체크합니다.
...
- 파괴적인 셸 명령어(`rm -rf`, `curl`)를 **거부(Denies)**합니다.
- 구현자 에이전트가 `npm test` 및 `npm run lint`를 실행하도록 **허용(Allows)**합니다.
- 그 외 모든 항목은 기본적으로 `ask_user`(사람의 승인 필요)로 **설정(Defaults)**합니다.
...
관리자 정책(시스템 수준에서 설정됨)은 다른 모든 정책보다 우선합니다. 이는 기업이 조직 전체의 가드레일을 강제하는 방법입니다.
...
**Workspace 계층 정책**(`.gemini/policies/dev.toml`의 리포지토리에 체크인됨)은 팀 수준의 기본값을 설정합니다:
...
> **활성 정책 검사:** CLI에서 `/policies list`를 사용하여 결정, 우선순위 계층 및 소스 파일을 포함하여 세션을 제어하는 모든 규칙을 확인하세요.
...
- **macOS 샌드박스**: macOS 샌드박스를 사용하여 파일 시스템 접근을 제한합니다.
...
최근 세션 목록을 표시합니다. 중단한 부분부터 계속할 세션을 선택하세요.
...
사용 가능한 사용자 지정 명령어를 표시합니다. `.gemini/commands/`에서 직접 정의할 수 있습니다.
...
> **파워 유저 및 재참여자를 위한 섹션입니다.** 이 섹션은 코드 생성을 넘어 리뷰, 문서화, 규정 준수 및 릴리스 관리를 포함하는 **전체 소프트웨어 개발 수명 주기(SDLC)**를 다룹니다. 각 에이전트는 독립적으로 사용할 수 있습니다. 어느 지점에서든 바로 시작해 보세요.
...
> **CI/CD에서 자동화하기:** 모든 풀 리퀘스트에 대한 자동화된 PR 리뷰를 위해 공식 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action을 사용하세요. CLI에서 `/setup-github`로 설치하면 워크플로 파일, 디스패치 핸들러 및 이슈 분류를 자동으로 구성합니다. 작동하는 예시는 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)을 참조하세요.
...
> **아우터 루프 가치:** 수 시간의 수동 문서화 작업을 대체합니다. 문서를 최신 상태로 유지하려면 각 스프린트 후에 실행하세요.

---

### 에이전트 3: 보안 분석 (공식 확장 프로그램)

맞춤형 규정 준수 검사기를 구축하는 대신 **공식 [보안 확장 프로그램](https://github.com/gemini-cli-extensions/security)**을 설치하세요. 이는 전체 SAST 엔진, [OSV-Scanner](https://github.com/google/osv-scanner)를 통한 종속성 스캔, 벤치마크된 성능(실제 CVE 대비 90% 정밀도, 93% 재현율)을 갖춘 Google에서 유지 관리하는 확장 프로그램입니다.
...
> **엔터프라이즈 가치:** 이 확장 프로그램은 PoC 생성(`poc`), 자동 패치(`security-patcher`) 및 취약점 허용 목록을 위한 스킬과 함께 제공됩니다. 즉시 프로덕션에 사용할 수 있으므로 맞춤형 규정 준수 에이전트를 구축할 필요가 없습니다.
...
진정한 강력함은 에이전트들을 하나의 워크플로로 결합하는 데 있습니다. 각 에이전트는 **새롭고 집중된 컨텍스트**를 얻게 되며, 단일 에이전트가 전체 대화 기록을 축적하지 않습니다:
...
> **이것이 효과적인 이유:** 각 단계는 특정 작업에 집중된 깔끔한 컨텍스트로 시작합니다. investigator는 구현 세부 정보를 전달하지 않습니다. 리뷰어는 조사 과정의 노이즈를 전달하지 않습니다. 이것이 모든 고성능 AI 워크플로의 이면에 있는 원칙입니다.
...
- [멀티 에이전트 오케스트레이션](advanced-patterns.md#multi-agent-orchestration)

---
## 2부 — 아우터 루프: 코드 작성을 넘어서

> **소요 시간:** 약 20분 (자율 진행)
> **사전 준비:** 위의 1부를 완료하세요. 사용자 정의 에이전트(§1.5) 및 Conductor(§1.4)에 익숙하면 도움이 됩니다.

위의 실습은 코드 작성, 테스트 및 리뷰와 같은 **이너 루프**에 중점을 두었습니다. 하지만 에이전트는 아키텍처 결정, 개발자 온보딩, 종속성 감사 및 CI 파이프라인 자동화와 같이 코드를 둘러싼 워크플로우인 **아우터 루프**도 처리할 수 있습니다.

1부에서 여러분은 이미 전문화된 역할을 위한 서브에이전트, 사양 주도 개발을 위한 Conductor, 정책 시행을 위한 규정 준수 검사기 등의 구성 요소를 구축했습니다. 2부에서는 이러한 패턴을 아우터 루프 워크플로우로 승격시키는 방법을 보여줍니다.

---

### 2.1 — 서브에이전트 주도 개발을 활용한 ADR 생성기

아키텍처 결정 기록(ADR)은 기술적 선택을 한 *이유*를 기록합니다. 수동으로 작성하는 것은 팀이 완전히 건너뛸 정도로 지루한 작업입니다. [superpowers 확장 프로그램](extensions-ecosystem.md#exercise-1-superpowers--methodology-as-extension)의 서브에이전트 주도 개발(SDD) 방법론을 사용하면 코드 변경 사항에서 ADR을 자동으로 생성할 수 있습니다.
...
코드 변경(기능 추가, 아키텍처 패턴 변경)을 수행한 후:
...
> **이것이 중요한 이유:** ADR은 팀이 생성할 수 있는 가장 가치 있는 산출물 중 하나이자 가장 방치되는 것 중 하나입니다. 모든 PR에서 초안 ADR을 생성하는 에이전트는 "문서 작성"에서 "문서 리뷰"로 장벽을 낮춥니다. 이 패턴을 채택하는 팀은 아키텍처 히스토리를 자동으로 구축합니다.

---

### 2.2 — 개발자 온보딩 에이전트

새로운 개발자는 기여하기 전에 코드베이스를 파악하는 데 며칠을 보냅니다. 온보딩 에이전트는 이 파악 작업을 몇 분 만에 수행합니다.
...
> **핵심 인사이트:** 이것을 README를 읽고 최신 상태이기를 바라는 것과 비교해 보세요. 에이전트는 변경되었을 수 있는 문서가 아니라 실제 코드 경로를 추적합니다. 이것은 1부(§1.5)의 `@codebase_investigator` 패턴이지만 온보딩 질문에 특화되어 있으며 재사용 가능한 에이전트로 유지됩니다.

---

### 2.3 — CI 파이프라인의 보안 분석

1부에서는 로컬 분석을 위해 [보안 확장 프로그램](https://github.com/gemini-cli-extensions/security)을 설치했습니다. 다음 단계는 이를 CI로 승격시키는 것입니다. 즉, 모든 풀 리퀘스트에 대한 자동화된 보안 분석입니다.

#### 패턴: GitHub Actions의 보안 확장 프로그램

보안 확장 프로그램은 즉시 사용할 수 있는 GitHub Actions 워크플로우와 함께 제공됩니다. 직접 복사하세요:
...
또는 [공식 워크플로우 템플릿](https://github.com/gemini-cli-extensions/security/blob/main/.github/workflows/gemini-review.yml)을 참조하여 수동으로 추가하세요. 워크플로우는 다음과 같습니다:

1. CI 러너에 보안 확장 프로그램 설치
2. PR diff에서 `/security:analyze` 실행
3. 종속성 취약점에 대해 `/security:scan-deps` 실행
4. 발견된 사항을 PR 댓글로 게시

**보안 확장 프로그램이 수작업 프롬프트보다 나은 이유:**

| 수작업 감사 프롬프트 | 보안 확장 프로그램 |
|---|---|
...
> **이것은 슬라이드 18의 CI 패턴입니다.** 하지만 수작업 프롬프트 대신 프로덕션 수준의 벤치마크된 확장 프로그램(정밀도 90%, 재현율 93%)을 사용합니다. §1.7에서 로컬로 실행했던 것과 동일한 `/security:analyze` 명령이 이제 모든 PR에서 자동으로 실행됩니다.
...
|---|---|
| 사용자 정의 서브에이전트 (§1.5) | ADR 작성기, 온보딩 가이드 |
| 보안 확장 프로그램 (§1.7) | CI 보안 분석 파이프라인 |
| Conductor 사양-코드 변환 (§1.4) | PRD → ADR → 구현 파이프라인 |
| 헤드리스 모드 (UC3에서 참조) | GitHub Action 자동화 |

패턴은 항상 동일합니다: **로컬에서 구축 → 검증 → CI/CD로 승격 → 조직 전체로 확장.** 한 명의 개발자를 돕는 에이전트가 전체 팀을 돕는 자동화가 됩니다.
...
| **JIT 컨텍스트 검색** | 현재 작업과 관련된 컨텍스트 파일만 로드합니다. |
| **메모리** | 세션 간에 지식을 유지합니다. |
| **Conductor** | 지속적인 계획 및 진행 상황 추적을 통한 사양 주도 개발입니다. |
| **확장 프로그램** | 스킬, 에이전트, 훅 및 정책의 설치 가능한 패키지입니다. |
| **MCP 서버** | 외부 도구(GitHub, BigQuery, Jira)에 연결합니다. |
| **정책 엔진** | TOML의 코드형 가드레일(Guardrails-as-code)입니다. deny, allow 또는 ask_user를 지원합니다. |
| **훅** | 에이전트 수명 주기 이벤트에서 경량 컨텍스트 주입 및 모델 스티어링을 수행합니다. |
| **샌드박싱** | 신뢰할 수 없는 환경을 위한 격리된 실행입니다. |
| **맞춤형 에이전트** | 단순한 코딩을 넘어 리뷰, 문서, 릴리스 노트를 위한 특화된 에이전트입니다. |
| **보안 확장 프로그램** | PoC 생성 및 자동 패치 기능이 포함된 공식 SAST + 종속성 검사입니다. |
| **내장 에이전트** | `generalist`, `codebase_investigator`, `cli_help` — 설정 없이 위임할 수 있습니다. |
| **ADR 생성** | git diff를 기반으로 서브에이전트가 주도하는 아키텍처 결정 기록(ADR)입니다. |
| **온보딩 에이전트** | 새로운 개발자를 위한 코드베이스 매핑입니다. 실제 코드 경로를 추적합니다. |
| **CI 보안 파이프라인** | 자동화된 취약점 분석을 위한 GitHub Actions의 보안 확장 프로그램입니다. |

---
## 다음 단계

→ **[사용 사례 2: 레거시 코드 현대화](legacy-modernization.md)**로 계속 진행합니다.
