...
> **소요 시간:** 약 60분  
> **목표:** 초기 설치부터 컨텍스트 엔지니어링, Conductor를 사용한 사양 주도 개발, 그리고 거버넌스 가드레일까지 아우르는 엔터프라이즈급 개발자 워크플로우를 구축합니다.  
...
> **진행 상황:** 에이전트가 `package.json`을 읽고, 디렉터리 구조를 스캔하며, 아키텍처를 매핑합니다. Gemini CLI는 필요에 따라 `read_file`, `glob`, `grep_search`와 같은 도구를 사용하여 파일을 읽고, 패턴을 검색하고, 종속성을 추적하면서 온디맨드 방식으로 코드베이스를 탐색합니다.
...
이는 에이전트가 사용할 수 있는 모든 도구(파일 작업, 셸 명령, 웹 검색 및 구성한 모든 MCP 서버)를 보여줍니다.
...
Gemini CLI는 여러 수준에서 `GEMINI.md` 파일을 읽으며, 각 파일은 더 구체적인 컨텍스트를 추가합니다:
...
- 아키텍처 규칙 (routes → controllers → models)
- 안티패턴 (콜백 금지, 하드코딩된 자격 증명 금지)
- 테스트 표준
...
> **예상 결과:** 에이전트는 이것이 GEMINI.md 규칙("라우트 파일에 비즈니스 로직 금지")을 위반한다는 것을 인식하고, 대신 컨트롤러에 엔드포인트를 생성하여 위임하는 얇은 라우트를 만들어야 합니다.

> **규칙 적용:** `GEMINI.md`가 강력한 지침(프롬프트 엔지니어링)을 제공하지만, AI는 복잡한 리팩터링 중에 여전히 환각을 일으키거나 실수를 할 수 있습니다. 진정으로 강력한 워크플로를 구축하려면 CI/CD 또는 [Gemini CLI 훅](https://geminicli.com/docs/hooks/)을 사용하여 이러한 프롬프트 기반 규칙을 결정론적 린터(`dependency-cruiser` 등)와 결합해야 합니다. 이를 설정하는 방법은 고급 패턴 가이드의 [결정론적 적용](advanced-patterns.md#deterministic-enforcement)을 참조하세요.
...
에이전트는 명시적으로 무언가를 기억하라고 요청할 때나, [settings.json](https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/auto-memory.md)에서 `experimental.autoMemory`를 활성화한 경우 자동으로 `save_memory` 도구를 사용하여 메모리를 직접 저장할 수도 있습니다.
...
> **이것이 중요한 이유:** `.geminiignore`가 없으면 에이전트가 `node_modules/`(수십만 개의 파일)를 읽는 데 컨텍스트 토큰을 낭비할 수 있습니다. 이를 사용하면 에이전트는 소스 코드에만 집중합니다.
...
플랜 모드는 일회성 기능에 매우 유용합니다. 하지만 영구적인 사양, 단계별 구현 계획, 세션 간 진행 상황 추적이 필요한 며칠에 걸친 프로젝트라면 Conductor가 적합합니다.
...
> **핵심 인사이트:** 이제 이 파일들은 프로젝트의 단일 진실 공급원(source of truth)입니다. 이 파일들은 마크다운(Markdown) 형식으로 리포지토리에 존재하며, 다른 코드와 마찬가지로 커밋되고 리뷰됩니다. 내일 다시 작업을 시작하거나 동료에게 이 프로젝트를 인계할 때, AI는 여러분이 중단한 바로 그 지점부터 작업을 재개합니다. 상태는 메모리가 아닌 파일에 저장됩니다.
...
> **계획을 살펴보세요.** 특정 작업과 체크박스가 포함된 단계별로 나뉘어 있습니다. 1단계: 데이터베이스 스키마. 2단계: API 엔드포인트. 3단계: 프론트엔드 컴포넌트. 4단계: 테스트. 에이전트는 이 계획을 순서대로 따르며, 진행하면서 작업을 체크합니다.

> **접근 방식에 동의하지 않는 경우** — 예를 들어 REST 대신 GraphQL을 원한다면 — `plan.md`를 직접 편집하고 다시 실행하세요. 이 계획은 여러분과 에이전트 간의 계약입니다.
...
> **온디맨드 탐색:** 에이전트는 계획의 각 단계를 구현할 때 도구를 통해 코드베이스를 탐색합니다 — 파일을 읽고, 임포트를 추적하고, 패턴을 상호 참조합니다. `GEMINI.md`와 같은 컨텍스트 파일과 Conductor 사양은 에이전트가 현재 작업 중인 파일과 함께 로드됩니다.
...
settings.json에는 GitHub MCP 서버가 포함되어 있습니다. `GITHUB_TOKEN`으로 구성된 경우 에이전트는 다음을 수행할 수 있습니다:
- 리포지토리, 이슈 및 PR 읽기
- 이슈 및 댓글 생성
- 풀 리퀘스트 열기
...
샘플 정책은 다음과 같습니다:
- `.env`, `.ssh` 및 자격 증명 파일 읽기를 **거부(Deny)**합니다.
- 파괴적인 셸 명령(`rm -rf`, `curl`)을 **거부(Deny)**합니다.
- 구현자 에이전트가 `npm test` 및 `npm run lint`를 실행하도록 **허용(Allow)**합니다.
- 그 외의 모든 항목은 `ask_user`(사람의 승인 필요)를 **기본값(Default)**으로 설정합니다.
...
관리자 정책(시스템 수준에서 설정됨)은 다른 모든 정책보다 우선합니다. 이것이 기업이 조직 전체의 가드레일을 강제하는 방식입니다.

> **참고:** 작업 공간(Workspace) 계층은 현재 CLI 소스에서 비활성화되어 있습니다. 최신 계층 상태는 [정책 엔진 참조](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)를 확인하세요.

### 훅 실행

`settings.json`에 구성된 훅은 이미 활성화되어 있습니다:

1. **SessionStart → session-context**: 이 세션이 시작될 때 브랜치 이름과 수정된(dirty) 파일 수를 주입했습니다.
...
> **설계 철학:** 이러한 훅은 가벼운 컨텍스트 주입기이자 모델 스티어링 도구이며, 무거운 테스트 실행기가 아닙니다. 총 지연 시간을 200ms 미만으로 추가하며, 시스템에 부담을 주지 않으면서 에이전트의 의사 결정 품질을 향상시킵니다.

### 엔터프라이즈 구성

조직 전체의 도구 제한을 위해서는 관리자 계층 TOML 정책과 함께 [정책 엔진](https://github.com/google-gemini/gemini-cli/blob/main/docs/reference/policy-engine.md)을 사용하세요. 실용적인 연습은 [정책 엔진으로 Gemini CLI 보호하기](https://aipositive.substack.com/p/secure-gemini-cli-with-the-policy)를 참조하세요.
...
**작업 공간 계층 정책**(`.gemini/policies/dev.toml`에 리포지토리로 체크인됨)은 팀 수준의 기본값을 설정합니다:
...
- **macOS 샌드박스**: macOS 샌드박스를 사용하여 파일 시스템 액세스를 제한합니다.
...
사용 가능한 커스텀 명령어를 표시합니다. `.gemini/commands/`에서 직접 정의할 수 있습니다.
...
| **메모리** | 세션 간에 지식을 유지합니다. |
| **Conductor** | 영구적인 계획 및 진행률 추적을 통한 사양 기반 개발을 지원합니다. |
...
| **샌드박스** | 신뢰할 수 없는 환경을 위한 격리된 실행을 제공합니다. |

---
## 1.7 — 전체 SDLC를 위한 맞춤형 에이전트 (20분)

> **파워 유저 및 재참여자를 위한 섹션입니다.** 이 섹션은 코드 생성을 넘어 리뷰, 문서화, 규정 준수 및 릴리스 관리를 포함한 **전체 소프트웨어 개발 수명 주기(SDLC)**를 다룹니다. 각 에이전트는 독립적으로 사용할 수 있습니다. 어느 시점에서든 바로 시작할 수 있습니다.

### 내장 에이전트

Gemini CLI는 즉시 사용할 수 있는 기본 에이전트와 함께 제공됩니다. 다음 명령어로 목록을 확인할 수 있습니다:
...
| 에이전트 | 목적 | 사용 시기 |
|---|---|---|
| **`generalist`** | 전체 도구에 액세스할 수 있는 일반 에이전트 | 대용량 또는 턴(turn)이 많이 필요한 작업 |
| **`codebase_investigator`** | 아키텍처 매핑 및 종속성 분석 | "이 앱에서 인증 흐름이 어떻게 되는지 매핑해 줘" |
| **`cli_help`** | Gemini CLI 문서 전문가 | "MCP 도구 격리를 어떻게 구성하나요?" |
...
> **이것이 중요한 이유:** investigator는 집중된 컨텍스트와 함께 읽기 전용 모드로 작동합니다. 아키텍처를 매핑하는 동안 실수로 파일을 수정하지 않습니다. 그런 다음 메인 에이전트는 해당 맵을 사용하여 구현을 계획합니다.

---

### 맞춤형 에이전트 구축

맞춤형 에이전트는 YAML 프런트매터가 포함된 Markdown 파일이며, `.gemini/agents/`에 배치됩니다. 각 에이전트는 다음을 갖습니다:

- `@agent-name`으로 호출하는 **이름**
- CLI가 자동 라우팅에 사용하는 **설명**
- 에이전트가 액세스할 수 있는 항목을 제어하는 **도구 허용 목록**
- 전문 지식과 출력 형식을 정의하는 **시스템 프롬프트**

> **핵심 설계 원칙:** 생각하는 역할과 실행하는 역할을 분리하세요. 조사 및 리뷰에는 읽기 전용 에이전트를 사용합니다. 구현에는 쓰기 액세스 에이전트를 사용합니다. 동일한 컨텍스트에서 조사와 변경을 절대 혼합하지 마세요.
...
> **CI/CD에서 자동화하기:** 모든 풀 리퀘스트에 대한 자동화된 PR 리뷰를 위해 공식 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action을 사용하세요. `/setup-github`을 사용하여 CLI에서 설치하면 워크플로우 파일, 디스패치 핸들러 및 이슈 분류가 자동으로 구성됩니다. 작동하는 예시는 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)을 참조하세요.

---

### 에이전트 2: 문서 작성자

소스 코드에서 API 문서, README 및 코드 주석을 생성합니다. 읽기 전용이며 파일을 절대 수정할 수 없습니다.
...
> **아우터 루프 가치:** 이것은 수 시간의 수동 문서화 작업을 대체합니다. 문서를 최신 상태로 유지하려면 각 스프린트 후에 실행하세요.

---

### 에이전트 3: 보안 분석 (공식 확장 프로그램)

맞춤형 규정 준수 검사기를 구축하는 대신, **공식 [보안 확장 프로그램](https://github.com/gemini-cli-extensions/security)**을 설치하세요. 이는 전체 SAST 엔진, [OSV-Scanner](https://github.com/google/osv-scanner)를 통한 종속성 스캐닝, 벤치마크된 성능(실제 CVE 대비 90% 정밀도, 93% 재현율)을 갖춘 Google 유지 관리 확장 프로그램입니다.
...
이 확장 프로그램은 현재 브랜치 diff에 대해 2단계 SAST 분석을 실행하여 다음을 확인합니다:
- 하드코딩된 시크릿 및 API 키
...
> **엔터프라이즈 가치:** 이 확장 프로그램은 PoC 생성(`poc`), 자동 패치(`security-patcher`) 및 취약점 허용 목록을 위한 스킬과 함께 제공됩니다. 즉시 프로덕션 환경에서 사용할 수 있으므로 맞춤형 규정 준수 에이전트를 구축할 필요가 없습니다.
...
> **이것이 효과적인 이유:** 각 단계는 특정 작업에 집중된 깨끗한 컨텍스트로 시작합니다. investigator는 구현 세부 정보를 전달하지 않습니다. reviewer는 조사 노이즈를 전달하지 않습니다. 이것이 모든 고성능 AI 워크플로우의 기본 원칙입니다.
...
- [워크트리를 사용한 병렬 개발](advanced-patterns.md#parallel-development-with-worktrees)
- [다중 에이전트 오케스트레이션](advanced-patterns.md#multi-agent-orchestration)

---
## 2부 — 아우터 루프: 코드 작성을 넘어서

> **소요 시간:** 약 20분 (자기 주도)
> **사전 준비:** 위의 1부를 완료하세요. 사용자 지정 에이전트(§1.5) 및 Conductor(§1.4)에 익숙하면 도움이 됩니다.

위의 실습은 코드 작성, 테스트 및 리뷰와 같은 **이너 루프**에 중점을 두었습니다. 하지만 에이전트는 아키텍처 결정, 개발자 온보딩, 종속성 감사 및 CI 파이프라인 자동화와 같이 코드를 둘러싼 워크플로인 **아우터 루프**도 처리할 수 있습니다.

1부에서는 이미 전문화된 역할을 위한 서브에이전트, 사양 주도 개발을 위한 Conductor, 정책 적용을 위한 규정 준수 검사기 등 구성 요소를 구축했습니다. 2부에서는 이러한 패턴을 아우터 루프 워크플로로 승격시키는 방법을 보여줍니다.

---

### 2.1 — 서브에이전트 주도 개발을 통한 ADR 생성기

아키텍처 결정 기록(ADR)은 기술적 선택을 한 *이유*를 기록합니다. 이를 수동으로 작성하는 것은 매우 번거로워서 팀에서 아예 생략하는 경우가 많습니다. [superpowers 확장 프로그램](extensions-ecosystem.md#exercise-1-superpowers--methodology-as-extension)의 서브에이전트 주도 개발(SDD) 방법론을 사용하면 코드 변경 사항에서 ADR을 자동으로 생성할 수 있습니다.
...
> **이것이 중요한 이유:** ADR은 팀이 생성할 수 있는 가장 가치 있는 산출물 중 하나이지만, 가장 방치되는 것 중 하나이기도 합니다. 모든 PR에서 초안 ADR을 생성하는 에이전트는 "문서 작성"에서 "문서 리뷰"로 장벽을 낮춰줍니다. 이 패턴을 채택하는 팀은 아키텍처 기록을 자동으로 구축하게 됩니다.

---

### 2.2 — 개발자 온보딩 에이전트

새로운 개발자는 기여하기 전에 코드베이스를 파악하는 데 며칠을 보냅니다. 온보딩 에이전트는 이 파악 작업을 단 몇 분 만에 수행합니다.
...
> **핵심 인사이트:** 이것을 README를 읽고 최신 상태이기를 바라는 것과 비교해 보세요. 에이전트는 실제 코드 경로를 추적하며, 내용이 달라졌을 수 있는 문서를 추적하지 않습니다. 이것은 1부(§1.5)의 `@codebase_investigator` 패턴이지만, 온보딩 질문에 특화되어 있고 재사용 가능한 에이전트로 유지됩니다.

---

### 2.3 — CI 파이프라인의 보안 분석

1부에서는 로컬 분석을 위해 [Security 확장 프로그램](https://github.com/gemini-cli-extensions/security)을 설치했습니다. 다음 단계는 이를 CI로 승격시켜 모든 풀 리퀘스트에 대해 자동화된 보안 분석을 수행하는 것입니다.

#### 패턴: GitHub Actions의 Security 확장 프로그램

Security 확장 프로그램은 즉시 사용 가능한 GitHub Actions 워크플로와 함께 제공됩니다. 직접 복사하세요:
...
또는 [공식 워크플로 템플릿](https://github.com/gemini-cli-extensions/security/blob/main/.github/workflows/gemini-review.yml)을 참조하여 수동으로 추가하세요. 이 워크플로는 다음과 같이 작동합니다:

1. CI 러너에 Security 확장 프로그램을 설치합니다.
2. PR diff에 대해 `/security:analyze`를 실행합니다.
3. 종속성 취약점에 대해 `/security:scan-deps`를 실행합니다.
4. 발견된 사항을 PR 댓글로 게시합니다.

**Security 확장 프로그램이 직접 작성한 프롬프트보다 뛰어난 이유:**

| 직접 작성한 감사 프롬프트 | Security 확장 프로그램 |
|---|---|
| 자유 형식 프롬프트 — 실행할 때마다 결과가 다름 | 일관된 방법론을 갖춘 구조화된 2패스 SAST 엔진 |
| 취약점 분류 체계 없음 | 7개 범주, 20개 이상의 취약점 유형, 심각도 기준(Critical/High/Medium/Low) |
| 종속성 스캔 없음 | Google의 취약점 데이터베이스에 대한 통합 OSV-Scanner |
| 수정 워크플로 없음 | 내장된 PoC 생성 및 자동 패치 스킬 |
...
1부에서는 서브에이전트, Conductor, 정책 엔진, 훅과 같은 구성 요소를 제공했습니다. 2부에서는 이러한 패턴을 아우터 루프로 승격시키는 방법을 보여주었습니다:

| 구성 요소 (1부) | 아우터 루프 애플리케이션 (2부) |
|---|---|
| 사용자 지정 서브에이전트 (§1.5) | ADR 작성기, 온보딩 가이드 |
| Security 확장 프로그램 (§1.7) | CI 보안 분석 파이프라인 |
| Conductor 사양-코드 변환 (§1.4) | PRD → ADR → 구현 파이프라인 |
| 헤드리스 모드 (UC3에서 참조됨) | GitHub Action 자동화 |

패턴은 항상 동일합니다: **로컬에서 빌드 → 검증 → CI/CD로 승격 → 조직 전체로 확장.** 한 명의 개발자를 돕는 에이전트가 전체 팀을 돕는 자동화가 됩니다.
...
| **확장 프로그램** | 스킬, 에이전트, 훅 및 정책으로 구성된 설치 가능한 패키지입니다. |
| **MCP 서버** | 외부 도구(GitHub, BigQuery, Jira)에 연결합니다. |
| **정책 엔진** | TOML 형식의 코드형 가드레일 — deny, allow 또는 ask_user를 지원합니다. |
| **훅** | 에이전트 수명 주기 이벤트에서 경량 컨텍스트 주입 및 모델 스티어링을 수행합니다. |
| **샌드박싱** | 신뢰할 수 없는 환경을 위한 격리된 실행을 제공합니다. |
| **맞춤형 에이전트** | 단순한 코딩을 넘어 리뷰, 문서, 릴리스 노트 등을 위한 특화된 에이전트입니다. |
| **보안 확장 프로그램** | PoC 생성 및 자동 패치 기능이 포함된 공식 SAST 및 종속성 검사입니다. |
| **내장 에이전트** | `generalist`, `codebase_investigator`, `cli_help` — 설정 없이 작업을 위임할 수 있습니다. |
| **ADR 생성** | git diff를 기반으로 서브에이전트가 주도하여 아키텍처 결정 기록(ADR)을 생성합니다. |
| **온보딩 에이전트** | 신규 개발자를 위한 코드베이스 매핑 — 실제 코드 경로를 추적합니다. |
| **CI 보안 파이프라인** | 자동화된 취약점 분석을 위해 GitHub Actions에서 보안 확장 프로그램을 사용합니다. |
