...
> **소요 시간:** 약 60분  
> **목표:** 초기 설치부터 컨텍스트 엔지니어링, Conductor를 사용한 사양 주도 개발, 그리고 거버넌스 가드레일에 이르기까지 엔터프라이즈급 개발자 워크플로우를 구축합니다.  
...
이렇게 하면 오류 처리, 비동기 패턴 및 보안에 대한 백엔드 전용 규칙이 추가됩니다.
...
플랜 모드는 일회성 기능에 적합합니다. 하지만 지속적인 사양, 단계별 구현 계획, 여러 세션에 걸친 진행 상황 추적이 필요한 며칠간의 프로젝트에는 Conductor가 제격입니다.
...
> **핵심 인사이트:** 이제 이 파일들은 프로젝트의 단일 진실 공급원(source of truth)이 됩니다. 이 파일들은 마크다운(Markdown) 형식으로 저장소에 존재하며, 다른 코드와 마찬가지로 커밋되고 리뷰됩니다. 내일 다시 돌아오거나 이 프로젝트를 동료에게 넘길 때, AI는 여러분이 중단한 바로 그 지점부터 다시 시작합니다. 상태는 메모리가 아닌 파일에 저장됩니다.
...
> **온디맨드 탐색:** 에이전트는 계획의 각 단계를 구현할 때 도구를 통해 코드베이스를 탐색합니다. 파일을 읽고, 임포트를 추적하고, 패턴을 상호 참조합니다. `GEMINI.md` 및 Conductor 사양과 같은 컨텍스트 파일은 에이전트가 활발하게 작업 중인 파일과 함께 로드됩니다.
...
- 그 외 모든 항목은 `ask_user`(사람의 승인 필요)를 **기본값(Defaults)**으로 설정합니다.
...
> **예상 결과:** 에이전트가 정책 엔진에 의해 차단되어야 합니다. 차단된 이유를 설명하는 거부 메시지가 표시됩니다.
...
관리자 정책(시스템 수준에서 설정됨)은 다른 모든 것을 재정의합니다. 이는 기업이 조직 전체의 가드레일을 강제하는 방법입니다.
...
1. **SessionStart → session-context**: 이 세션 시작 시 브랜치 이름과 변경된 파일(dirty file) 수를 주입했습니다.
...
> **설계 철학:** 이러한 훅은 무거운 테스트 러너가 아니라 가벼운 컨텍스트 주입기이자 모델 스티어링 도구입니다. 총 지연 시간을 200ms 미만으로 추가하며 시스템에 부담을 주지 않으면서 에이전트의 의사 결정 품질을 향상시킵니다.
...
| **메모리** | 세션 간에 지식을 유지합니다 |
| **Conductor** | 지속적인 계획 및 진행 상황 추적을 통한 사양 주도 개발 |
...
> **파워 유저 및 재참여자를 위한 섹션입니다.** 이 섹션은 코드 생성을 넘어 리뷰, 문서화, 규정 준수 및 릴리스 관리를 포함하는 **전체 소프트웨어 개발 수명 주기(SDLC)**를 다룹니다. 각 에이전트는 독립적으로 사용할 수 있습니다. 어느 시점에서든 바로 시작할 수 있습니다.
...
| 에이전트 | 목적 | 사용 시기 |
|---|---|---|
| **`generalist`** | 전체 도구 접근 권한을 가진 일반 에이전트 | 대용량 또는 턴이 많이 필요한 작업 |
...
> **이것이 중요한 이유:** 조사자(investigator)는 집중된 컨텍스트를 가지고 읽기 전용 모드로 작동합니다. 아키텍처를 매핑하는 동안 실수로 파일을 수정하지 않습니다. 그런 다음 메인 에이전트가 해당 맵을 사용하여 구현을 계획합니다.
...
- 에이전트가 접근할 수 있는 대상을 제어하는 **도구 허용 목록(tool allowlist)**
- 전문 지식과 출력 형식을 정의하는 **시스템 프롬프트(system prompt)**

> **핵심 설계 원칙:** 생각하는 역할(thinkers)과 실행하는 역할(doers)을 분리하세요. 조사 및 리뷰에는 읽기 전용 에이전트를 사용합니다. 구현에는 쓰기 권한이 있는 에이전트를 사용합니다. 동일한 컨텍스트에서 조사와 변경(mutation)을 절대 혼합하지 마세요.

아래 예시들은 Gemini CLI가 단순한 코드 생성기가 아니라 리뷰, 문서화, 규정 준수 및 릴리스 관리를 포괄하는 **전체 SDLC 플랫폼**임을 보여줍니다.

---

### 에이전트 1: PR 리뷰어

코드 변경 사항의 품질, 버그 및 스타일 위반을 검토하는 읽기 전용 에이전트입니다.
...
> **CI/CD에서 자동화하기:** 모든 풀 리퀘스트(PR)에 대한 자동화된 PR 리뷰를 원한다면, 공식 [`google-github-actions/run-gemini-cli`](https://github.com/google-github-actions/run-gemini-cli) GitHub Action을 사용하세요. CLI에서 `/setup-github` 명령어로 설치하면 워크플로 파일, 디스패치 핸들러, 이슈 분류가 자동으로 구성됩니다. 작동하는 예시는 [`samples/cicd/gemini-pr-review.yml`](../../samples/cicd/gemini-pr-review.yml)을 참조하세요.
...
> **아우터 루프 가치:** 이는 수 시간의 수동 문서화 작업을 대체합니다. 문서를 최신 상태로 유지하기 위해 각 스프린트 후에 실행하세요.

---

### 에이전트 3: 보안 분석 (공식 확장 프로그램)

맞춤형 규정 준수 검사기를 구축하는 대신, **공식 [보안 확장 프로그램(Security Extension)](https://github.com/gemini-cli-extensions/security)**을 설치하세요. 이는 Google에서 유지 관리하는 확장 프로그램으로, 전체 SAST 엔진, [OSV-Scanner](https://github.com/google/osv-scanner)를 통한 종속성 스캐닝, 그리고 벤치마크된 성능(실제 CVE 대비 90% 정밀도, 93% 재현율)을 제공합니다.
...
> **엔터프라이즈 가치:** 이 확장 프로그램은 PoC 생성(`poc`), 자동 패치(`security-patcher`) 및 취약점 허용 목록(allowlisting)을 위한 스킬과 함께 제공됩니다. 기본적으로 프로덕션 환경에 바로 사용할 수 있으므로 맞춤형 규정 준수 에이전트를 구축할 필요가 없습니다.

---

### 에이전트 4: 릴리스 노트 작성자

git 기록과 변경된 파일을 읽어 구조화되고 이해관계자 친화적인 릴리스 노트를 생성합니다.
...
> **아우터 루프 가치:** 릴리스 노트는 SDLC 작업 중 가장 꺼려지는 작업 중 하나입니다. 이 에이전트는 git 기록과 실제 코드 변경 사항을 모두 읽어 제품 관리자가 이해할 수 있는 노트를 생성합니다.
...
> **이것이 효과적인 이유:** 각 단계는 특정 작업에 집중된 깨끗한 컨텍스트로 시작합니다. 조사자(investigator)는 구현 세부 정보를 전달하지 않습니다. 리뷰어는 조사 과정의 노이즈를 전달하지 않습니다. 이것이 모든 고성능 AI 워크플로의 이면에 있는 원칙입니다.
...
---
## 파트 2 — 아우터 루프: 코드 작성을 넘어서

> **소요 시간:** 약 20분 (자기 주도)
> **사전 요구 사항:** 위의 파트 1을 완료하세요. 사용자 지정 에이전트(§1.5) 및 Conductor(§1.4)에 익숙하면 도움이 됩니다.

위의 실습은 코드 작성, 테스트 및 검토와 같은 **이너 루프**에 중점을 두었습니다. 하지만 에이전트는 코드를 둘러싼 워크플로우인 **아우터 루프**(아키텍처 결정, 개발자 온보딩, 종속성 감사, CI 파이프라인 자동화)도 처리할 수 있습니다.

파트 1에서 여러분은 이미 전문화된 역할을 위한 서브에이전트, 사양 주도 개발을 위한 Conductor, 정책 시행을 위한 규정 준수 검사기와 같은 구성 요소를 구축했습니다. 파트 2에서는 이러한 패턴을 아우터 루프 워크플로우로 승격시키는 방법을 보여줍니다.

---

### 2.1 — 서브에이전트 주도 개발을 활용한 ADR 생성기

아키텍처 결정 기록(ADR)은 기술적 선택을 한 *이유*를 기록합니다. 수동으로 작성하는 것은 매우 번거롭기 때문에 팀에서 이를 완전히 생략하는 경우가 많습니다. [슈퍼파워 확장 프로그램](extensions-ecosystem.md#exercise-1-superpowers--methodology-as-extension)의 서브에이전트 주도 개발(SDD) 방법론을 사용하면 코드 변경 사항에서 자동으로 ADR을 생성할 수 있습니다.
...
> **이것이 중요한 이유:** ADR은 팀이 생성할 수 있는 가장 가치 있는 산출물 중 하나이지만, 가장 방치되는 것 중 하나이기도 합니다. 모든 PR에서 초안 ADR을 생성하는 에이전트는 장벽을 "문서 작성"에서 "문서 검토"로 낮춰줍니다. 이 패턴을 채택한 팀은 아키텍처 히스토리를 자동으로 구축하게 됩니다.
...
> **핵심 인사이트:** 이것을 README를 읽고 최신 상태이기를 바라는 것과 비교해 보세요. 에이전트는 실제 코드 경로를 추적하며, 내용이 달라졌을 수 있는 문서를 추적하지 않습니다. 이것은 파트 1(§1.5)의 `@codebase_investigator` 패턴과 동일하지만, 온보딩 질문에 특화되어 있고 재사용 가능한 에이전트로 지속된다는 점이 다릅니다.

---

### 2.3 — CI 파이프라인에서의 보안 분석

파트 1에서는 로컬 분석을 위해 [보안 확장 프로그램](https://github.com/gemini-cli-extensions/security)을 설치했습니다. 다음 단계는 이를 CI로 승격시키는 것입니다. 즉, 모든 풀 리퀘스트에 대해 자동화된 보안 분석을 수행하는 것입니다.

#### 패턴: GitHub Actions의 보안 확장 프로그램

보안 확장 프로그램은 바로 사용할 수 있는 GitHub Actions 워크플로우와 함께 제공됩니다. 이를 직접 복사하세요:
...
4. 발견된 결과를 PR 댓글로 게시합니다.

**보안 확장 프로그램이 수작업으로 작성한 프롬프트보다 뛰어난 이유:**
...
> **이것은 슬라이드 18의 CI 패턴입니다.** 하지만 수작업으로 작성한 프롬프트 대신 프로덕션 수준의 벤치마크된 확장 프로그램(정밀도 90%, 재현율 93%)을 사용합니다. §1.7에서 로컬로 실행했던 것과 동일한 `/security:analyze` 명령이 이제 모든 PR에서 자동으로 실행됩니다.

---

### 종합하기

파트 1에서는 서브에이전트, Conductor, 정책 엔진, 훅과 같은 구성 요소를 제공했습니다. 파트 2에서는 이러한 패턴을 아우터 루프로 승격시키는 방법을 보여주었습니다:

| 구성 요소 (파트 1) | 아우터 루프 애플리케이션 (파트 2) |
|---|---|
...
| **JIT 컨텍스트 검색** | 현재 작업과 관련된 컨텍스트 파일만 로드합니다 |
| **메모리** | 세션 간에 지식을 유지합니다 |
| **Conductor** | 지속적인 계획 및 진행률 추적을 통한 사양 기반 개발 |
| **확장 프로그램** | 스킬, 에이전트, 훅 및 정책의 설치 가능한 패키지 |
| **MCP 서버** | 외부 도구(GitHub, BigQuery, Jira)에 연결합니다 |
...
| **보안 확장 프로그램** | 공식 SAST + PoC 생성 및 자동 패치를 포함한 종속성 검사 |
