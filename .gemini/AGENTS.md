...
│   ├── validate.py            ← 5-check structural validation
│   ├── drift.py               ← Manifest-based drift detection
...
### Modifying translated content (docs/ko/, docs/ja/, etc.)
1. **NEVER edit files in `docs/ko/` or any `docs/{lang}/` directory directly**
2. These are generated by `make translate L=xx` — manual edits will be overwritten
3. To fix a translation issue, either:
   - Fix the English source in `docs/` and re-run the pipeline
   - Update the glossary in `tools/i18n/glossary-{lang}.md` and re-run
4. To check translation freshness: `make translate-drift L=ko`
5. The manifest at `tools/i18n/.translation-manifest.json` tracks what was
   translated from what — do not edit it manually
6. See `tools/i18n/TRANSLATE.md` for the full translation guide
